...
class Monetary(Field):
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-tigernix: currency may not be already initialized if it is
...
class Datetime(Field):
    ...
    @staticmethod
    def context_utctimestamp(record, timestamp):
        """Return the given timestamp converted to the client's timezone.
        ...
        if tz_name:
            try:
                context_tz = pytz.timezone(tz_name)
                return context_tz.localize(timestamp).astimezone(pytz.utc)
            except Exception:
                _logger.debug("failed to compute context/client-specific timestamp, "
                              "using the UTC value",
                              exc_info=True)
...
class Binary(Field):
    ...
    def convert_to_column(self, value, record, values=None, validate=True):
        ...
        # the legacy TigernixERP convention is to transfer and store binaries
...
class Many2many(_RelationalMulti):
    ...
    def read(self, records):
        ...
        order_by = getattr(self,'query_order',None) or comodel._generate_order_by(None, wquery)
