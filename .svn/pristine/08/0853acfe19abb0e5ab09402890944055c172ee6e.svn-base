...
(function () {
    ...
    tigernix.testing = typeof QUnit === 'object';
    tigernix.remainingJobs = jobs;
    tigernix.__DEBUG__ = {
    ...
    tigernix.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__tigernix_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: tigernix.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = tigernix.__DEBUG__.getDependencies(job.name);
            ...
            missing = tigernix.__DEBUG__.getMissingJobs();
            failed = tigernix.__DEBUG__.getFailedJobs();
            ...
            if (tigernix.debug || failed.length || unloaded.length) {
                ...
                if (tigernix.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        tigernix.__DEBUG__.jsModules = {
            missing: missing,
    ...
    tigernix.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            tigernix.processJobs(jobs, services);
