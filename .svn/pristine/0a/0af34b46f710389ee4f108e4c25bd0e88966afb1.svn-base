...
from tigernix import fields, tools, SUPERUSER_ID
from tigernix.tools.pycompat import to_text
from tigernix.http import request
from tigernix.modules.module import get_resource_path
from .qweb import escape
import psycopg2
from tigernix.tools import func, misc
        ...
        return """
            (function (message) {
                ...
                var loaded = function () {
                    ...
                    var tigernix = window.top.tigernix;
                    if (!tigernix || !tigernix.define) return;

                    tigernix.define("AssetsBundle.ErrorMessage", function (require) {
                        "use strict";
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In TigernixERP you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
