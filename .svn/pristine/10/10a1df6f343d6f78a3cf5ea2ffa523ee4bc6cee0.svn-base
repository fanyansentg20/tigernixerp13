...
"""
Miscellaneous tools used by TigernixERP.
"""
...
from tigernix.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if tigernix.tools.config['db_host']:
        env['PGHOST'] = tigernix.tools.config['db_host']
    if tigernix.tools.config['db_port']:
        env['PGPORT'] = str(tigernix.tools.config['db_port'])
    if tigernix.tools.config['db_user']:
        env['PGUSER'] = tigernix.tools.config['db_user']
    if tigernix.tools.config['db_password']:
        env['PGPASSWORD'] = tigernix.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the TigernixERP root, using a subdir folder.
    ...
    """
    import tigernix.modules as addons
    adps = tigernix.addons.__path__
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the TigernixERP root: skip zipfile lookup.
...
def _fileopen(path, mode, basedir, pathinfo, basename=None):
    ...
    import tigernix.modules as addons
    paths = tigernix.addons.__path__ + [config['root_path']]
...
def flatten(list):
    ...
    Author: Christophe Simonis (christophe@tigernixerp.com)
...
def scan_languages():
    """ Returns all languages supported by TigernixERP for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = tigernix.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns
       ...
       the evaluation.
       Mostly useful for evaluating TigernixERP domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('tigernix.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('tigernix.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = tigernix.fields.Datetime.from_string(value)
            value = tigernix.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = tigernix.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = tigernix.fields.Datetime.from_string(value)
    else:
        timestamp = value
