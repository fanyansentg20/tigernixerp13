...
import tigernix
import tigernix.modules.db
import tigernix.modules.graph
import tigernix.modules.migration
import tigernix.modules.registry
import tigernix.tools as tools

from tigernix import api, SUPERUSER_ID
from tigernix.modules.module import adapt_version, initialize_sys_path, load_tigernixerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('tigernix.tests')
...
def force_demo(cr):
    ...
    """
    graph = tigernix.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(idref, mode):
        ...
        try:
            ...
            tigernix.registry(cr.dbname).clear_caches()
    ...
    processed_modules = []
    loaded_modules = []
    registry = tigernix.registry(cr.dbname)
    migrations = tigernix.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        load_tigernixerp_module(package.name)

        new_install = package.state == 'to install'
        if new_install:
            py_module = sys.modules['tigernix.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = tigernix.tools.config["overwrite_existing_translations"]
            ...
            if tools.config.options['test_enable']:
                ...
                report.record_result(tigernix.modules.module.run_unit_tests(module_name))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, tigernix.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        ...
        # tigernix.modules.registry.Registry.new().
        registry = tigernix.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['tigernix.addons.%s' % (pkg.name,)]
                ...
                registry = tigernix.modules.registry.Registry.new(
