...
import tigernix
from tigernix import api, fields, models, SUPERUSER_ID
from tigernix.tools.misc import DEFAULT_SERVER_DATETIME_FORMAT
from tigernix.tools import date_utils
...
class ImBus(models.Model):
    ...
    def sendmany(self, notifications):
        ...
        if channels:
            ...
            def notify():
                with tigernix.sql_db.db_connect('postgres').cursor() as cr:
...
class ImDispatch(object):
    ...
    def poll(self, dbname, channels, last, options=None, timeout=TIMEOUT):
        ...
        if not tigernix.evented:
            current = threading.current_thread()
            current._daemonic = True
            # rename the thread to avoid tests waiting for a longpolling
            current.setName("tigernixerp.longpolling.request.%s" % current.ident)
...
dispatch = None
if not tigernix.multi_process or tigernix.evented:
