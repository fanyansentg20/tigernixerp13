tigernix.define('web.WebClient', function (require) {
"use strict";
...
return AbstractWebClient.extend({
    ...
    bind_events: function () {
        ...
        /*
            Small patch to allow having a link with a href towards an anchor. Since TigernixERP use hashtag
    ...
    load_menus: function () {
        return (tigernix.loadMenusPromise || tigernix.reloadMenus())
            .then(function (menuData) {
                // Compute action_id if not defined on a top menu item
                // Add a condition to check menuData and menuData.children before returning menuData (Updated on October 16, 2024)
                if (menuData && menuData.children) {
                    ...
                    tigernix.loadMenusPromise = null;
                    return menuData;
                }
            });
    },
