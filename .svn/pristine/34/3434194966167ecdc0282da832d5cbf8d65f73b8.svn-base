...
from tigernix import _, api, models
from tigernix.exceptions import UserError
from tigernix.osv import expression
from tigernix.tools.misc import format_date, get_lang
...
class Base(models.AbstractModel):
    ...
    def _get_date_column_label(self, date, field, span, step):
        """
            ...
            :param field: tigernix.field object of the current model
        """
        ...
        if step == 'week':
            ...
            def _week_format(date):
                if field.type == 'date':
                    ...
                    # Check https://github.com/python-babel/babel/issues/619 and change this when TigernixERP will use a fixed Babel version
    ...
    def _grid_range_of(self, span, step, anchor, field):
        """
            ...
            :param field: `tigernix.field` used as grouping criteria
        """
        if field.type == 'datetime':
