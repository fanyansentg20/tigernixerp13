tigernix.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousTigernix: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        var tigernixSuggestions = [];
        var clearbitSuggestions = [];
        return new Promise(function (resolve, reject) {
            var tigernixPromise = self._getTigernixSuggestions(value, isVAT).then(function (suggestions){
                tigernixSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with TigernixERP result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = tigernixSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            tigernixSuggestions.push(suggestion);
                        }
                    });
                }

                tigernixSuggestions = _.filter(tigernixSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(tigernixSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(tigernixSuggestions);
            };

            self._whenAll([tigernixPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    /**
     * Use TigernixERP Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
    ...
    _getTigernixSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousTigernix.add(def);
    },
    /**
