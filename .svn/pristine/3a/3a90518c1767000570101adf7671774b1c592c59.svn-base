...
from tigernix import _
from tigernix.exceptions import UserError

from tigernix.addons.payment.models.payment_acquirer import _partner_split_name
...
class AuthorizeAPI():
    ...
    def create_customer_profile(self, partner, opaqueData):
        ...
        a corresponding payment profile to it. Note that a single partner in the TigernixERP
        ...
        values = {
            'createCustomerProfileRequest': {
                ...
                'profile': {
                    'description': ('TIGERNIX-%s-%s' % (partner.id, uuid4().hex[:8]))[:20],
    ...
    def create_customer_profile_from_tx(self, partner, transaction_id):
        ...
        values = {
            'createCustomerProfileFromTransactionRequest': {
                ...
                'customer': {
                    'merchantCustomerId': ('TIGERNIX-%s-%s' % (partner.id, uuid4().hex[:8]))[:20],
                    'email': partner.email or ''
                }
            }
        }
