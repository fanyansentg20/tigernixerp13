...
class SurveyQuestion(models.Model):
    ...
    #=================
    #Added on 13 Feb 2025 by Yudha
    #Fix issue where sequence is set as 10 on creation
    @api.model
    def create(self, vals):
        """ Ensure sequence is properly managed when creating a new question """
        if 'sequence' not in vals:
            last_question = self.search([('survey_id', '=', vals.get('survey_id'))], order='sequence desc', limit=1)
            vals['sequence'] = (last_question.sequence + 10) if last_question else 10
    ...
    def _compute_page_id(self):
        ...
        for question in self:
            if question.is_page:
                ...
                #==================
                #==================
                #Modified on 13 Feb 2025 by Yudha
                #fix issue where page_id is missing upon creation
                pages = question.survey_id.page_ids.sorted(lambda p: p.sequence)
                assigned_page = next((page for page in reversed(pages) if page.sequence <= question.sequence), None)
                question.page_id = assigned_page
                #==================
                #==================
                # Original Code
