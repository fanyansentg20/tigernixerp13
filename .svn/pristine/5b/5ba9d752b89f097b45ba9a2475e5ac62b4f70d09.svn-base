...
from tigernix.addons.iap import jsonrpc
from tigernix import api, exceptions, fields, models, _
from tigernix.exceptions import AccessError, ValidationError
from tigernix.tests.common import Form
from tigernix.tools.misc import clean_context
import logging
import re

_logger = logging.getLogger(__name__)

PARTNER_REMOTE_URL = 'https://partner-autocomplete.tigernix.com/iap/partner_autocomplete'
...
class AccountMove(models.Model):
    ...
    def retry_ocr(self):
        ...
        if attachments and attachments.exists() and self.extract_state in ['no_extract_requested', 'not_enough_credit', 'error_status', 'module_not_up_to_date']:
            ...
            endpoint = self.env['ir.config_parameter'].sudo().get_param(
                'account_invoice_extract_endpoint', 'https://iap-extract.tigernix.com') + '/iap/invoice_extract/parse'
            user_infos = {
    ...
    def post(self):
        ...
        for record in self.filtered(lambda move: move.is_invoice()):
            if record.extract_state == 'waiting_validation':
                endpoint = self.env['ir.config_parameter'].sudo().get_param(
                    'account_invoice_extract_endpoint', 'https://iap-extract.tigernix.com') + '/iap/invoice_extract/validate'
                values = {
    ...
    def _check_status(self):
        self.ensure_one()
        endpoint = self.env['ir.config_parameter'].sudo().get_param(
            'account_invoice_extract_endpoint', 'https://iap-extract.tigernix.com') + '/iap/invoice_extract/get_result'
    ...
    def _save_form(self, ocr_results, no_ref=False):
        ...
        with Form(self_ctx) as move_form:
            ...
            # if the total on the invoice doesn't match the total computed by TigernixERP, adjust the taxes so that it matches
