...
import tigernix
from tigernix import api, fields, models, modules, tools, _
from tigernix.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from tigernix.exceptions import AccessDenied, UserError
from tigernix.osv import expression
from tigernix.tools.parse_version import parse_version
from tigernix.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'TigernixERP Enterprise Edition License v1.0'),
        ('OPL-1', 'TigernixERP Proprietary License v1.0'),
    ...
    to_buy = fields.Boolean('TigernixERP Enterprise Module', default=False)
    ...
    def install_from_urls(self, urls):
        ...
        TIGERNIXERP = tigernix.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info('Downloading module `%s` from TigernixERP Apps', module_name)
            ...
            for module_name, url in urls.items():
                if module_name == TIGERNIXERP or not url:
                    continue    # TIGERNIXERP is special case, handled below, and no URL means local module
            ...
            if urls.get(TIGERNIXERP):
                ...
                # copy all modules in the SERVER/tigernix/addons directory to the new "tigernix" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, TIGERNIXERP, 'addons', d)    # XXX 'tigernix' subdirectory ?
                ...
                _logger.info('Copy downloaded module `tigernix` to `%s`', server_dir)
                shutil.move(os.path.join(tmp, TIGERNIXERP), server_dir)
                #if bck:
                #    shutil.rmtree(bck)
            ...
            if installed or to_install:
                ...
                self._cr.commit()
                tigernix.service.server.restart()
                return {
                    'type': 'ir.actions.client',
                    'tag': 'home',
                    'params': {'wait': True},
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.tigernix.com/apps')
