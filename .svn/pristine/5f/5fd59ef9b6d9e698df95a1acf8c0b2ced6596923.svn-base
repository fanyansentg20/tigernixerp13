tigernix.define("web.Domain", function (require) {
"use strict";
        ...
        if (this._data === true || this._data === false) {
            ...
            switch (this._data[1]) {
                ...
                case ">=":
                    return (fieldValue >= this._data[2]);
                case "in":
                    var inList = _.isArray(this._data[2]) ? this._data[2] : [this._data[2]];
                    // For x2many: check if any ID in fieldValue exists in the condition list
                    if (_.isArray(fieldValue)) {
                        return _.some(fieldValue, function(val) {
                            return _.contains(inList, val);
                        });
                    }
                    return _.contains(inList, fieldValue);
                case "not in":
                    var notInList = _.isArray(this._data[2]) ? this._data[2] : [this._data[2]];
                    // For x2many: check if no ID in fieldValue exists in the condition list
                    if (_.isArray(fieldValue)) {
                        return !_.some(fieldValue, function(val) {
                            return _.contains(notInList, val);
                        });
                    }
                    return !_.contains(notInList, fieldValue);
                case "like":
