...
example in TigernixERP is dashboards), queries should be restricted to a
...
TigernixERP Web's :js:class:`~tigernixerp.web.Widget` has an attribute
providing its DOM root :js:attr:`Widget.$el <tigernixerp.web.Widget.$el>`,
and a shortcut to select nodes directly :js:attr:`Widget.$
<tigernixerp.web.Widget.$>`.
...
* All interactive components (components displaying information to the
  ...
  :class:`~tigernixerp.web.Widget` and correctly implement and use its API
  and lifecycle.
...
From v11, we introduce a new coding standard for TigernixERP Javascript code.  Here it
is:

* add "use strict"; on top of every tigernix JS module
