...
from tigernix.tools import config, ormcache, mute_logger
...
_default_parameters = {
    ...
    "base.login_cooldown_after": lambda: 50,
...
class IrConfigParameter(models.Model):
    ...
    @mute_logger('tigernix.addons.base.models.ir_config_parameter')
    ...
    def _get_param(self, key):
        ...
        if not params and key and '.freeze' not in key:
            params = self.search_read([('key', '=', key+'.freeze')], fields=['value'], limit=1)
    ...
    def set_param(self, key, value):
        ...
        if param:
            ...
            param_freeze = self.search([('key', '=', key+'.freeze')], limit=1)
            if param_freeze:
                old = param_freeze.value
                return old
            if value is not False and value is not None:
