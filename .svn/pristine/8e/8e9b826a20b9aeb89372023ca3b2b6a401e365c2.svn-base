...
TigernixERP Web 7.0 implements a unified facets-based search view instead
...
makes significant use of Backbone's models and collections (TigernixERP
Web's widgets make a good replacement for Backbone's own views). As a
result, understanding the implementation details of the TigernixERP Web 7
...
The primary component of search views, as with all other TigernixERP
views, are inputs. The search view has two types of inputs — filters
and fields — but only one is easly customizable: fields.

The mapping from TigernixERP field types (and widgets) to search view
objects is stored in the ``tigernixerp.web.search.fields``
:js:class:`~tigernixerp.web.Registry` where new field types and widgets
can be added.
...
:js:func:`~tigernixerp.web.search.Input.facet_for_defaults` on each input,
passing it a mapping (a javascript object) of ``name:value`` extracted
from the action's context.

This method should fetch a :js:class:`~tigernixerp.web.search.Facet` (or
...
:js:func:`tigernixerp.web.search.Input.facet_for` with that value.

There is no default implementation of
:js:func:`tigernixerp.web.search.Input.facet_for` [#no_impl]_, but
:js:class:`tigernixerp.web.search.Field` provides one, which uses the
value as-is to fetch a :js:class:`~tigernixerp.web.search.Facet`.
...
inputs through the :js:func:`~tigernixerp.web.search.Input.complete`
method.
...
``facet``
    ...
    Either a :js:class:`~tigernixerp.web.search.Facet` object or (more
...
.. note::
    ...
    <tigernixerp.web.search.Input.visible>`, its completion function will
    *not* be called.
...
:js:func:`~tigernixerp.web.search.Input.in_drawer`.

By default, :js:func:`~tigernixerp.web.search.Input.in_drawer` returns the
value of :js:attr:`~tigernixerp.web.search.Input._in_drawer`, which is
``false``. The behavior can be toggled either by redefining the
attribute to ``true`` (either on the class or on the input), or by
overriding :js:func:`~tigernixerp.web.search.Input.in_drawer` itself.
...
.. note::
    ...
    An :js:func:`invisible <tigernixerp.web.search.Input.visible>` input
...
TigernixERP this is done via :ref:`domains <tigernixerpserver:domains>`. On
the other hand, the TigernixERP Web 7 search view's state is modelled
after a collection of :js:class:`~tigernixerp.web.search.Facet`, and each
...
:js:class:`~tigernixerp.web.search.Facet` objects to TigernixERP search data.

This is done via an input's
:js:func:`~tigernixerp.web.search.Input.get_domain` and
:js:func:`~tigernixerp.web.search.Input.get_context`. Each takes a
:js:class:`~tigernixerp.web.search.Facet` and returns whatever it's
...
context, and can throw an :js:class:`~tigernixerp.web.search.Invalid`
exception if the value is not valid at all for the field.

.. note::

    The :js:class:`~tigernixerp.web.search.Facet` object can have any
    number of values (from 1 upwards)

.. note::

    There is a third conversion method,
    :js:func:`~tigernixerp.web.search.Input.get_groupby`, which returns an
...
:js:class:`~tigernixerp.web.search.SearchQuery` available as
:js:attr:`tigernixerp.web.SearchView.query`.

The query is a `backbone collection`_ of
:js:class:`~tigernixerp.web.search.Facet` objects, which can be interacted
with directly by external objects or search view controls
(e.g. widgets displayed in the drawer).

.. js:class:: tigernixerp.web.search.SearchQuery

    The current search query of the search view, provides convenience
    behaviors for manipulating :js:class:`~tigernixerp.web.search.Facet`
    on top of the usual `backbone collection`_ methods.

    The query ensures all of its facets contain at least one
    :js:class:`~tigernixerp.web.search.FacetValue` instance. Otherwise,
    the facet is automatically removed from the query.

    .. js:function:: tigernixerp.web.search.SearchQuery.add(values, options)
    ...
    .. js:function:: tigernixerp.web.search.SearchQuery.toggle(value, options)
...
.. js:class:: tigernixerp.web.search.Facet
    ...
    .. js:attribute:: field
        ...
        The :js:class:`~tigernixerp.web.search.Input` instance which
    ...
    .. js:attribute:: values
        ...
        :js:class:`~tigernixerp.web.search.FacetValues` as a javascript
...
.. js:class:: tigernixerp.web.search.FacetValues

    `Backbone collection`_ of
    :js:class:`~tigernixerp.web.search.FacetValue` instances.

.. js:class:: tigernixerp.web.search.FacetValue
    ...
    .. js:attribute:: value
        ...
        :js:class:`~tigernixerp.web.search.Input` to serialize to domains
        and contexts.
...
:js:class:`~tigernixerp.web.search.Field` provides a default
implementation of :js:func:`~tigernixerp.web.search.Input.get_domain` and
:js:func:`~tigernixerp.web.search.Input.get_context` taking care of most
of the peculiarities pertaining to TigernixERP's handling of fields in
...
:js:func:`~tigernixerp.web.search.Input.get_domain` or
:js:func:`~tigernixerp.web.search.Input.get_context`:

.. js:function:: tigernixerp.web.search.Field.get_context(facet)

    If the field has no ``@context``, simply returns
    ``null``. Otherwise, calls
    :js:func:`~tigernixerp.web.search.Field.value_from` once for each
    :js:class:`~tigernixerp.web.search.FacetValue` of the current
    :js:class:`~tigernixerp.web.search.Facet` (in order to extract the
    basic javascript object from the
    :js:class:`~tigernixerp.web.search.FacetValue` then evaluates
    ``@context`` with each of these values set as ``self``, and
    returns the union of all these contexts.

    :param facet:
    :type facet: tigernixerp.web.search.Facet
    :returns: a context (literal or compound)

.. js:function:: tigernixerp.web.search.Field.get_domain(facet)

    If the field has no ``@filter_domain``, calls
    :js:func:`~tigernixerp.web.search.Field.make_domain` once with each
    :js:class:`~tigernixerp.web.search.FacetValue` of the current
    :js:class:`~tigernixerp.web.search.Facet` as well as the field's
    ``@name`` and either its ``@operator`` or
    :js:attr:`~tigernixerp.web.search.Field.default_operator`.

    If the field has an ``@filter_value``, calls
    :js:func:`~tigernixerp.web.search.Field.value_from` once per
    :js:class:`~tigernixerp.web.search.FacetValue` and evaluates
    ``@filter_value`` with each of these values set as ``self``.

    In either case, "ors" all of the resulting domains (using ``|``)
    if there is more than one
    :js:class:`~tigernixerp.web.search.FacetValue` and returns the union
    of the result.
...
.. js:function:: tigernixerp.web.search.Field.make_domain(name, operator, facetValue)

    Builds a literal domain from the provided data. Calls
    :js:func:`~tigernixerp.web.search.Field.value_from` on the
    :js:class:`~tigernixerp.web.search.FacetValue` and evaluates and sets
    ...
    :param facetValue:
    :type facetValue: tigernixerp.web.search.FacetValue
    :returns: Array<(String, String, Object)>

.. js:function:: tigernixerp.web.search.Field.value_from(facetValue)

    Extracts a "bare" javascript value from the provided
    :js:class:`~tigernixerp.web.search.FacetValue`, and returns it.
...
.. js:attribute:: tigernixerp.web.search.Field.default_operator

    Operator used to build a domain when a field has no ``@operator``
    or ``@filter_domain``. ``"="`` for
    :js:class:`~tigernixerp.web.search.Field`

Arbitrary data storage
----------------------

:js:class:`~tigernixerp.web.search.Facet` and
:js:class:`~tigernixerp.web.search.FacetValue` objects (and structures)
...
TigernixERP Web 6.1 to TigernixERP Web 7.
...
* :js:func:`tigernixerp.web.SearchView.do_clear` has been removed
* :js:func:`tigernixerp.web.SearchView.do_toggle_filter` has been removed

Widgets API
+++++++++++

* :js:func:`tigernixerp.web.search.Widget.render` has been removed

* :js:func:`tigernixerp.web.search.Widget.make_id` has been removed

* Search field objects are not tigernixerp widgets anymore, their
  ``start`` is not generally called

* :js:func:`~tigernixerp.web.search.Input.clear` has been removed since
  clearing the search view now simply consists of removing all search
  facets

* :js:func:`~tigernixerp.web.search.Input.get_domain` and
  :js:func:`~tigernixerp.web.search.Input.get_context` now take a
  :js:class:`~tigernixerp.web.search.Facet` as parameter, from which it's
  their job to get whatever value they want

* :js:func:`~tigernixerp.web.search.Input.get_groupby` has been added. It returns
  an :js:class:`Array` of context-like constructs. By default, it does not do
  anything in :js:class:`~tigernixerp.web.search.Field` and it returns the various
  contexts of its enabled filters in
  :js:class:`~tigernixerp.web.search.FilterGroup`.

Filters
+++++++

* :js:func:`tigernixerp.web.search.Filter.is_enabled` has been removed

* :js:class:`~tigernixerp.web.search.FilterGroup` instances are still
...
* ``get_value`` has been replaced by
  :js:func:`~tigernixerp.web.search.Field.value_from` as it now takes a
  :js:class:`~tigernixerp.web.search.FacetValue` argument (instead of no
  argument). It provides a default implementation returning the
  ``value`` property of its argument.

* The third argument to
  :js:func:`~tigernixerp.web.search.Field.make_domain` is now a
  :js:class:`~tigernixerp.web.search.FacetValue` so child classes have all
...
* Because the autocompletion service is now provided by the search
  view itself,
  :js:func:`tigernixerp.web.search.ManyToOneField.setup_autocomplete` has
  been removed.
...
* The advanced search is now a more standard
  :js:class:`~tigernixerp.web.search.Input` configured to be rendered in
  the drawer.

* :js:class:`~tigernixerp.web.search.ExtendedSearchProposition.Field` are
  now standard widgets, with the "right" behaviors (they don't rebind
  their ``$element`` in ``start()``)

* The ad-hoc optional setting of the tigernixerp field descriptor on a
  :js:class:`~tigernixerp.web.search.ExtendedSearchProposition.Field` has
  been removed, the field descriptor is now passed as second argument
  to the
  :js:class:`~tigernixerp.web.search.ExtendedSearchProposition.Field`'s
  constructor, and bound to its
  :js:attr:`~tigernixerp.web.search.ExtendedSearchProposition.Field.field`.

* Instead of its former domain triplet ``(field, operator, value)``,
  :js:func:`~tigernixerp.web.search.ExtendedSearchProposition.get_proposition`
...
.. [#facet-field]
    ...
    :js:class:`~tigernixerp.web.search.Input`, nor does it need to be what
    created the facet, it just needs to provide the three
    facet-serialization methods
    :js:func:`~tigernixerp.web.search.Input.get_domain`,
    :js:func:`~tigernixerp.web.search.Input.get_context` and
    :js:func:`~tigernixerp.web.search.Input.get_gropuby`, existing
    :js:class:`~tigernixerp.web.search.Input` subtypes merely provide
    ...
    :js:class:`~tigernixerp.web.search.Input`, as
    :js:class:`~tigernixerp.web.search.CustomFilters` and
    :js:class:`~tigernixerp.web.search.Advanced` do.
