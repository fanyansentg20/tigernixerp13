...
import os
from tigernix.tools import misc
from pathlib import Path

from uuid import getnode as get_mac
from tigernix.addons.hw_proxy.controllers import main as hw_proxy
from tigernix.addons.web.controllers import main as web
from tigernix.modules.module import get_resource_path
from tigernix.addons.hw_drivers.tools import helpers
from tigernix.addons.hw_drivers.controllers.driver import iot_devices
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('tigernix.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('tigernix-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_tigernix_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'tigernix-remote-server.conf'
        if (wifi.exists() == False or remote_server.exists() == False) and helpers.access_point():
            return configure_wizard_template.render({
                ...
                'server': helpers.get_tigernix_server_url(),
    ...
    def list_drivers(self):
        ...
        return driver_list_template.render({
            'title': "TigernixERP's IoT Box - Drivers list",
    ...
    def load_drivers(self):
        ...
        subprocess.check_call(["sudo", "service", "tigernix", "restart"])
    ...
    def list_credential(self):
        return list_credential_template.render({
            'title': "TigernixERP's IoT Box - List credential",
            'breadcrumb': 'List credential',
            'db_uuid': helpers.read_file_first_line('tigernix-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('tigernix-enterprise-code.conf'),
        })
    ...
    def clear_credential(self):
        helpers.unlink_file('tigernix-db-uuid.conf')
        helpers.unlink_file('tigernix-enterprise-code.conf')
        subprocess.check_call(["sudo", "service", "tigernix", "restart"])
        return "<meta http-equiv='refresh' content='20; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        if server:
            res_payload['server'] = {
                'url': server,
                'message': 'Redirect to TigernixERP Server'
            }
        else:
            res_payload['server'] = {
    ...
    def clear_server_configuration(self):
        helpers.unlink_file('tigernix-remote-server.conf')
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_tigernix_server_url()
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> TigernixERP server configuration',
            'breadcrumb': 'Configure TigernixERP Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_tigernix_server_url() or 'Not configured yet',
    ...
    def add_six_payment_terminal(self, terminal_id):
        helpers.write_file('tigernix-six-payment-terminal.conf', terminal_id)
        subprocess.check_call(["sudo", "service", "tigernix", "restart"])
        return 'http://' + helpers.get_ip() + ':8069'

    @http.route('/six_payment_terminal_clear', type='http', auth='none', cors='*', csrf=False)
    def clear_six_payment_terminal(self):
        helpers.unlink_file('tigernix-six-payment-terminal.conf')
        subprocess.check_call(["sudo", "service", "tigernix", "restart"])
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
