#tigernix.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.tigernixerp.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        }
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.tigernixerp13rc (or to ~/.tigernixerp_serverrc if it exists)")
        ...
        group.add_option("--P", dest="prevent_crawling", help="True or False", my_default=True)

        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store TigernixERP data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'tigernix_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "tigernix.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="tigernix.http.rpc.request:DEBUG", help='shortcut for --log-handler=tigernix.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="tigernix.http.rpc.response:DEBUG", help='shortcut for --log-handler=tigernix.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="tigernix.http:DEBUG", help='shortcut for --log-handler=tigernix.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="tigernix.sql_db:DEBUG", help='shortcut for --log-handler=tigernix.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options. ",
            "Use these options to translate TigernixERP to another language. "
        ...
        if os.name == 'posix':
            ...
            group.add_option("--workers", dest="workers", my_default=0,
                             ...
                             type="int")
            group.add_option("--limit-memory-soft", dest="limit_memory_soft", my_default=20480 * 10240 * 10240,
                             help="Maximum allowed virtual memory per worker, when reached the worker be "
                             "reset after the current request (default 2048MiB).",
                             type="int")
            group.add_option("--limit-memory-hard", dest="limit_memory_hard", my_default=25600 * 10240 * 10240,
                             help="Maximum allowed virtual memory per worker, when reached, any memory "
                             "allocation will fail (default 2560MiB).",
                             type="int")
            group.add_option("--limit-time-cpu", dest="limit_time_cpu", my_default=6000,
                             help="Maximum allowed CPU time per request (default 6000).", #original is 60, changed to 6000
                             type="int")
            group.add_option("--limit-time-real", dest="limit_time_real", my_default=36000,
                             help="Maximum allowed Real time per request (default 36000).", #original is 120, changed to 36000
                             type="int") #--limit-time-real will kill any process if run over the configured seconds, including when upgrade module, import data, etc
    ...
    def parse_config(self, args=None):
        ...
        This method initializes tigernix.tools.config and tigernixerp.conf (the
        ...
        Typical usage of this method:
            ...
            tigernix.tools.config.parse_config(sys.argv[1:])
        """
        self._parse_config(args)
        tigernix.netsvc.init_logger()
        tigernix.modules.module.initialize_sys_path()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'tigernix.conf')
        else:
            rcfilepath = os.path.expanduser('~/.tigernixerp13rc')
            old_rcfilepath = os.path.expanduser('~/.tigernixerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.tigernixerp13rc' and '.tigernixerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.tigernixerp13rc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('TIGERNIX_RC') or os.environ.get('TIGERNIXERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from tigernix.modules.module import MANIFEST_NAMES
