...
import tigernix
import tigernix.tools as tools
import tigernix.release as release
from tigernix import SUPERUSER_ID, api
from tigernix.tools import pycompat

MANIFEST_NAMES = ('__manifest__.py', '__tigernixerp__.py')
README = ['README.rst', 'README.md', 'README.txt']

_logger = logging.getLogger(__name__)

# addons path as a list
# ad_paths is a deprecated alias, please use tigernix.addons.__path__
@tools.lazy
def ad_paths():
    _logger.warning('"tigernix.modules.module.ad_paths" is a deprecated '
                    'proxy to "tigernix.addons.__path__". Please consider '
                    'using the latter as the former is going to be '
                    'removed in the next version.',
                    exc_info=DeprecationWarning(), stack_info=True)
    return tigernix.addons.__path__
hooked = False
...
class AddonsHook(object):
    """ Makes modules accessible through tigernixerp.addons.* """

    def find_module(self, name, path=None):
        if name.startswith('tigernixerp.addons.') and name.count('.') == 2:
            _logger.warning('"tigernixerp.addons" is a deprecated alias to '
                            '"tigernix.addons". Please consider using the '
    ...
    def load_module(self, name):
        ...
        tigernix_name = re.sub(r'^tigernixerp.addons.(\w+)$', r'tigernix.addons.\g<1>', name)

        tigernix_module = sys.modules.get(tigernix_name)
        if not tigernix_module:
            tigernix_module = importlib.import_module(tigernix_name)

        sys.modules[name] = tigernix_module

        return tigernix_module
...
class TigernixHook(object):
    """ Makes tigernix package also available as tigernixerp """

    def find_module(self, name, path=None):
        # tigernixerp.addons.<identifier> should already be matched by AddonsHook,
        # only framework and subdirectories of modules should match
        if re.match(r'^tigernixerp\b', name):
            _logger.warning('tigernixerp is a deprecated alias to tigernix. '
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^tigernixerp(.*)', r'tigernix\g<1>', name)
...
def initialize_sys_path():
    """
    Setup an import-hook to be able to import TigernixERP addons from the different
    addons paths.

    This ensures something like ``import crm`` (or even
    ``import tigernix.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global hooked

    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in tigernix.addons.__path__:
        tigernix.addons.__path__.append(dd)

    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in tigernix.addons.__path__:
            tigernix.addons.__path__.append(ad)
    ...
    if base_path not in tigernix.addons.__path__ and os.path.isdir(base_path):
        tigernix.addons.__path__.append(base_path)

    if not hooked:
        sys.meta_path.insert(0, TigernixHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    initialize_sys_path()
    for adp in tigernix.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = tigernix.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in tigernix.addons.__path__:
...
def load_tigernixerp_module(module_name):
    """ Load an TigernixERP module, if not already loaded.
    ...
    initialize_sys_path()
    try:
        __import__('tigernix.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['tigernix.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    initialize_sys_path()
    for ad in tigernix.addons.__path__:
...
def get_test_modules(module):
    ...
    modpath = 'tigernix.addons.' + module
    try:
...
class TigernixTestResult(unittest.result.TestResult):
    """
    ...
    def getErrorCallerInfo(self, error, test):
        ...
        # only test case should be executed in tigernix, this is only a safe guard
...
def run_unit_tests(module_name, position='at_install'):
    ...
    """
    global current_test
    from tigernix.tests.common import TagsSelector # Avoid import loop
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            result = TigernixTestRunner().run(suite)
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, tigernix.sql_db.sql_counter - t0_sql)
