...
A very basic TigernixERP module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An TigernixERP
...
.. note::
    ...
    restart the TigernixERP server itself for it to be taken in account.
...
provides: an instance of the TigernixERP Web client, with the contents of
...
its :js:func:`~tigernixerp.web.Widget.start` to add some content to its
DOM:
...
will require restarting the TigernixERP Server to see the changes, as
usual):
...
.. note::
    ...
    Prefixing CSS rules with both ``.tigernixerp`` (to ensure the rule
    will apply only within the confines of the TigernixERP Web client) and
...
be done in :js:func:`~tigernixerp.web.Widget.start` but that gets unwieldy
...
TigernixERP Web's template language is :doc:`qweb`. Although any
...
integration to TigernixERP Web widgets.
...
To prevent this irremediable loss, we can use TigernixERP's support for
...
:js:func:`~tigernixerp.base.Widget.start()` is to perform *asynchronous*
...
recorded previously using the :js:class:`~tigernixerp.web.Query` interface
...
4. And finally — after restarting TigernixERP — navigating to the test
...
side-effect of this is to :js:func:`~tigernixerp.web.Widget.start` it,
...
:js:func:`set a widget's DOM <tigernixerp.web.Widget.setElement>`, let's
