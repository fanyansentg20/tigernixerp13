define([
  ...
  var Editor = function (handler) {
    ...
    this.style = style;   // TIGERNIX: allow access for override
    this.table = table;   // TIGERNIX: allow access for override
    this.typing = typing; // TIGERNIX: allow access for override
    this.bullet = bullet; // TIGERNIX: allow access for override
    ...
    this.saveRange = function ($editable, thenCollapse) {
      // TIGERNIX: scroll to top when click on input in editable m (start_modification
      // this.focus($editable);
      var r = range.create();
      if (!r || ($editable[0] !== r.sc && !$.contains($editable[0], r.sc))) {
        $editable.focus();
      }
      // TIGERNIX: end_modication)
    ...
    this.createLink = function ($editable, linkInfo, options) {
      ...
      var anchors = [];
      // TIGERNIX: adding this branch to modify existing anchor if it fully contains the range
      ...
      $.each(anchors, function (idx, anchor) {
        ...
        $(anchor).attr('class', linkInfo.className || null); // TIGERNIX: addition
        $(anchor).css(linkInfo.style || {}); // TIGERNIX: addition
        if (isNewWindow) {
