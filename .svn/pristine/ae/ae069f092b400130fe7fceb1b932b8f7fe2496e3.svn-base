...
def is_initialized(cr):
    ...
    """
    return tigernix.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = tigernix.modules.get_module_resource('base', 'data', 'base_data.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with tigernix.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())

    for i in tigernix.modules.get_modules():
        mod_path = tigernix.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = tigernix.modules.load_information_from_description_file(i)
...
def has_unaccent(cr):
    ...
    module but any similar function will be picked by TigernixERP.
