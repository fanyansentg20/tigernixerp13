tigernix.define('project_timeshee.ui', function (require ) {
    "use strict";
    ...
    // Desktop detection
    // In TigernixERP, Awesome Timesheet is embedded inside an iframe.
    ...
    var ProjectTimesheet = Widget.extend(ServiceProviderMixin, {
        ...
        */
        ...
        // TigernixERP XML ids require a specific format : module.id.
                                ...
                                .then(function (ids) {
                                    if (ids.length === 0) { // there are no dirty ids on the server
                                        ...
                                        if (!self.data.warning_date || (self.data.warning_date && moment(self.data.warning_date).diff(new Date(), 'days') !== 0)) {
                                            alert('The code on your TigernixERP server is not up to date and an important update has been released. You or your system administrator should consider retrieving it as soon as possible.');
    ...
    var Sync_screen = BasicScreenWidget.extend({
            ...
            _.extend(self.events,
                {
                    ...
                    "click .pt_tigernix_login_link" : "select_tigernix_login",
        ...
        select_tigernix_login: function() {
            var self = this;
            self.server = 'https://www.tigernix.com';
            session.origin = self.server;
            session.setup(self.server, {use_cors : true});
            var always = function() {
                if (session.uid) {
        ...
        show_login_screen: function() {
            this.tigernix_login_screen = new Tigernix_login_screen(this);
            this.$('div.pt_sync_screen').empty();
            this.tigernix_login_screen.appendTo(this.$('div.pt_sync_screen'));
        },
                ...
                if (response.length > 0 && _.contains(response, 'project_timesheet_synchro')) {
                    ...
                    session.rpc('/jsonrpc',  { method : 'server_version' , service : 'db', args : []}).then(function(result) {
                        if (result && result.endsWith('e')) {
                            ...
                            alert("Timesheet Synchronization is available in TigernixERP Enterprise Edition. You should consider upgrading your TigernixERP version if you would like to use it.");
                        } else {
                            alert("The server does not support timesheet synchronization. It requires TigernixERP Enterprise Edition version 9 or newer.");
                        }
                    })
                    self.on_keep_data();
                }
    ...
    var Tigernix_login_screen = Widget.extend({
        template : "tigernix_saas_login",
        events: {
            "click .pt_send_tigernix_login" : "send_tigernix_login",
        },
        send_tigernix_login: function() {
            var self = this;
            var login = this.$(".pt_tigernix_login").val();
            var password = this.$(".pt_tigernix_password").val();
            var db_name = 'tigernixerp';
            var server_address = 'https://www.tigernix.com';
    ...
    var Db_selector_screen = Widget.extend({
        ...
        willStart: function() {
            var self = this;
            return this._rpc({
                    model: 'tigernixerp.enterprise.database',
            ...
            this._rpc({
                ...
                .then(function(res) {
                    var state =  JSON.stringify({
                        ...
                        'p':1, // 1 is the code to use TigernixERP as provider
                    });
                    ...
                    $.ajax({
                        ...
                        var always = function() {
                            if (session.uid) {
                                ...
                                alert('TigernixERP login failed');
                            }
                        }
                        session.session_reload().then(always).guardedCatch(always);
                    }).guardedCatch(function(res) {
    ...
    var Premise_login_url_screen = Widget.extend({
        ...
        validate_url: function() {
            ...
            if (server_address === "tigernix.com" || server_address === "www.tigernix.com") {
                server_address = "www.tigernix.com";
                self.protocol = "https://";
            }
            session.origin = protocol + server_address;
            session.setup(protocol + server_address, {use_cors : true});

            if (server_address === "www.tigernix.com") {
                self.getParent().db_list = ["tigernixerp"];
                self.getParent().show_premise_login_form_screen();
                return;
    ...
    var Account_creation_screen = Widget.extend({
        ...
        init: function(parent) {
            ...
            self.tigernix_is_online = false;
            window.addEventListener("message", this.received_message, false);
            core.bus.on('db_created', this, this.on_db_creation_success);
        },
        /**
         * We perform a quick check (before rendering the widget) to see if TigernixERP is reachable. If not, we don't display an iframe with an ugly 404 message.
        ...
        */
        willStart: function() {
            ...
            return new Promise(function (resolve, reject) {
                $.ajax({
                    url: 'https://www.tigernix.com/fr_FR/trial',
                    type: 'HEAD',
                    cache: false,
                    timeout: 5000,
                    error: function() {
        ...
        received_message: function(event) {
            if (event.origin === 'https://www.tigernix.com' && event.data === 'success') {
