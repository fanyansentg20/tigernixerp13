...
from tigernix.exceptions import RedirectWarning, UserError
from tigernix.tools.safe_eval import safe_eval
from tigernix.tools.translate import _

from tigernix.addons.google_account.models.google_service import GOOGLE_TOKEN_ENDPOINT, TIMEOUT
...
class GoogleDrive(models.Model):
    ...
    def copy_doc(self, res_id, template_id, name_gdocs, res_model):
        ...
        if content.get('alternateLink'):
            ...
            request_url = "https://www.googleapis.com/drive/v2/files/%s/permissions?emailMessage=This+is+a+drive+file+created+by+TigernixERP&sendNotificationEmails=false&access_token=%s" % (key, access_token)
