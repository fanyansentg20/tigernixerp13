tigernix.define('web.data', function (require) {
"use strict";
 ...
 * consumed by TigernixERP's RPC APIs.
 *
...
var Query = Class.extend({
     ...
     * @returns {jQuery.Deferred<Array<tigernixerp.web.QueryGroup>> | null}
     */
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of TigernixERP records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the TigernixERP model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // TigernixERP ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // TigernixERP ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, TigernixERP domain
     ...
     * an SQL-like form usable by TigernixERP's ORM.
     *
     ...
     * TigernixERP's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
...
var Model = Class.extend({
    /**
    new tigernixerp.web.Model(name[, context[, domain]])

    @constructs instance.web.Model
    @extends instance.web.Class

    @param {String} name name of the TigernixERP model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound TigernixERP model.
     *
