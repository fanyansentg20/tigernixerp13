tigernix.define('web.mixins', function (require) {
"use strict";
...
function TigernixEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
TigernixEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};

TigernixEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
var EventDispatcherMixin = _.extend({}, ParentedMixin, {
     ...
     * TigernixERP Web where developers may want to replace existing callbacks with
     * theirs.
     *
    ...
    trigger_up: function (name, info) {
        var event = new TigernixEvent(this, name, info);
