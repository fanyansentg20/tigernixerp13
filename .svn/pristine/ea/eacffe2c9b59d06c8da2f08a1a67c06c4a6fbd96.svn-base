...
from tigernix.addons.payment_authorize.controllers.main import AuthorizeController
...
@tigernix.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('tigernix.addons.payment_authorize.models.payment', 'ValidationError')
...
@tigernix.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeS2s(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on tigernix/tigernix + create sandbox account
        authorize.write({
