tigernix.define('web_mobile.ActionManager', function (require) {
"use strict";
...
/*
    We don't want to open website urls in the TigernixERP apps (iOS and Android).
    The apps detect the redirection and open the url in a seprate browser.

    In TigernixERP desktop, the redirection occurs in the same tab and the returned
    promise is never resolved.
    This override returns a resolved promise in case of mobile app redirects
    because TigernixERP is not aware of this and we need to reactivate status button.
