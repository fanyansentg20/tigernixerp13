...
DEFAULT_ENDPOINT = 'https://ocn.tigernix.com'
...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_endpoint(self):
        return self.env['ir.config_parameter'].sudo().get_param('tigernix_ocn.endpoint', DEFAULT_ENDPOINT)

    @api.model
    def get_fcm_project_id(self):
        ir_params_sudo = self.env['ir.config_parameter'].sudo()
        project_id = ir_params_sudo.get_param('tigernix_ocn.project_id')
        if not project_id:
            params = {
            ...
            ir_params_sudo.set_param('tigernix_ocn.project_id', project_id)
        return project_id
