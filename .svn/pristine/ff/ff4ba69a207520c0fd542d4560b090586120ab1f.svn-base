...
from tigernix.exceptions import UserError, AccessError
...
from tigernix.sql_db import TestCursor
from tigernix.http import request
from tigernix.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of TigernixERP so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at TigernixERP start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start TigernixERP with at least two workers to print a pdf version of the reports.')
