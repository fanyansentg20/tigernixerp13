...
from tigernix.tools.float_utils import float_round as round, float_compare
from tigernix.tools import DEFAULT_SERVER_DATETIME_FORMAT
from tigernix.exceptions import UserError, ValidationError
from tigernix import api, fields, models, _, tools
from tigernix.tests.common import Form
...
class AccountJournal(models.Model):
    ...
    invoice_reference_model = fields.Selection(string='Communication Standard', required=True, selection=[('tigernixerp', 'TigernixERP'),('euro', 'European')], default='tigernixerp', help="You can choose different models for each type of reference. The default one is the TigernixERP reference.")
    ...
    inbound_payment_method_ids = fields.Many2many('account.payment.method', 'account_journal_inbound_payment_method_rel', 'journal_id', 'inbound_payment_method',
        ...
        help="Manual: Get paid by cash, check or any other method outside of TigernixERP.\n"\
             "Electronic: Get paid automatically through a payment acquirer by requesting a transaction on a card saved by the customer when buying or subscribing online (payment token).\n"\
             "Batch Deposit: Encase several customer checks at once by generating a batch deposit to submit to your bank. When encoding the bank statement in TigernixERP,you are suggested to reconcile the transaction with the batch deposit. Enable this option from the settings.")
    outbound_payment_method_ids = fields.Many2many('account.payment.method', 'account_journal_outbound_payment_method_rel', 'journal_id', 'outbound_payment_method',
        ...
        help="Manual:Pay bill by cash or any other method outside of TigernixERP.\n"\
             "Check:Pay bill by check and print it from TigernixERP.\n"\
