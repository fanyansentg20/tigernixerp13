...
from tigernix.exceptions import MissingError, UserError, ValidationError, AccessError
from tigernix.osv import expression
from tigernix.tools.safe_eval import safe_eval, test_python_expr
from tigernix.tools import wrap_module
...
class IrActionsServer(models.Model):
    ...
    Since TigernixERP 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: TigernixERP Environment on which the action is triggered
#  - model: TigernixERP Model of the record on which the action is triggered; is a void recordset
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': tigernix.exceptions.Warning,
            # record
            'record': record,
            'records': records,
            # helpers
