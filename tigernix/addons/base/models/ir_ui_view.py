...
from tigernix.modules.module import get_resource_from_path, get_resource_path
from tigernix.tools import config, graph, ConstantMapping, pycompat, apply_inheritance_specs, locate_node
from tigernix.tools.convert import _fix_multiple_roots
from tigernix.tools.json import scriptsafe as json_scriptsafe
from tigernix.tools.safe_eval import safe_eval
from tigernix.tools.view_validation import valid_view, get_attrs_field_names, field_is_editable
from tigernix.tools.translate import xml_translate, TRANSLATED_ATTRS
from tigernix.tools.image import image_data_uri
...
_ALLOWED_NODES = (
    ...
    ast.Load, ast.Tuple, ast.List, ast.Constant, ast.Num, ast.Str,
    ast.NameConstant, ast.Subscript, ast.Index, ast.Attribute, ast.Call,
...
def _leaf_to_expr(leaf):
    """Convert a single domain leaf to expression."""
    field, op, value = leaf
    ...
    op_map = {
        ...
        'in': 'in',
        'not in': 'not in',
        'like': 'like',  # Not directly convertible
        'ilike': 'ilike',  # Not directly convertible
    }
    ...
    if isinstance(value, str):
        ...
        value_repr = repr(tuple(value)) if op in ('in', 'not in') else repr(value)
    else:
        value_repr = repr(value)

    return "%s %s %s" % (field, py_op, value_repr)


def _format_suggestion(node, attr, suggested_expr):
    """Format a helpful suggestion message."""
    tag = node.tag
    name = node.get('name', '')

    if name:
        return '<%s name="%s" %s="%s" />' % (tag, name, attr, suggested_expr)
    else:
        return '<%s %s="%s" />' % (tag, attr, suggested_expr)

def _domain_to_expr(domain):
    """
    Convert an Odoo domain to a Python expression string.
    Used for generating migration suggestions.
    """
    ...
    if len(domain) == 1 and isinstance(domain[0], (list, tuple)):
        return _leaf_to_expr(domain[0])

    # Parse domain with operators
    def parse(domain, index=0):
        if index >= len(domain):
            return None, index
        ...
        if item == '&':
            left, next_idx = parse(domain, index + 1)
            right, final_idx = parse(domain, next_idx)
            return "(%s and %s)" % (left, right), final_idx

        elif item == '|':
            left, next_idx = parse(domain, index + 1)
            right, final_idx = parse(domain, next_idx)
            return "(%s or %s)" % (left, right), final_idx

        elif item == '!':
            inner, next_idx = parse(domain, index + 1)
            return "(not %s)" % inner, next_idx

        elif isinstance(item, (list, tuple)) and len(item) == 3:
            return _leaf_to_expr(item), index + 1

        return str(item), index + 1

    try:
        result, _ = parse(domain)
        return result or 'True'
    except Exception:
        # Fallback: return domain as-is for complex cases
        return repr(domain)

def get_expression_field_names(expr):
    """Extract field names from expression (excluding 'context')."""
    try:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Name) and node.id not in ('context', 'True', 'False'):
            fields.add(node.id)
    return fields


def _is_static_bool(expr):
    """Check if expression is a static boolean."""
    return expr.strip() in ('0', '1', 'True', 'False', 'true', 'false')


def _is_context_only(expr):
    """Check if expression only uses context (no record fields)."""
    try:
        tree = ast.parse(expr, mode='eval')
    except SyntaxError:
        return False
...
def _eval_context_expr(expr, context):
    """Evaluate a context-only expression."""
    try:
        return bool(safe_eval(expr, {'context': context or {}}))
    except Exception:
        return False
...
def _ast_to_value(node):
    ...
    raise ValidationError("Cannot convert AST node to value: %s" % ast.dump(node))


def _negate_domain(domain):
    """Negate a domain."""
    if not domain:
        return [(1, '=', 0)]  # Always false

    # Single leaf: simple inversion
    if len(domain) == 1 and isinstance(domain[0], tuple):
        field, op, value = domain[0]
        if op == '=' and value in (True, False):
            return [(field, '=', not value)]
        if op in _OP_INVERSE:
            return [(field, _OP_INVERSE[op], value)]

    # Complex domain: use '!' prefix
    return ['!'] + domain


def _expr_to_domain(expr, model_fields, modifier=None):
    """Convert field expression to Odoo domain.

    Args:
        expr: The expression string
        model_fields: Dictionary of model field definitions
        modifier: The modifier name (e.g., 'column_invisible', 'invisible', etc.)
                  When 'column_invisible', allows parent.xxx field references.
    """
    try:
        tree = ast.parse(expr, mode='eval').body
    except SyntaxError as e:
        raise ValidationError("Invalid expression syntax: %s" % e)

    def _is_x2many_field(field_name):
        """Check if field is One2many or Many2many."""
        if not model_fields:
            return False
        field_def = model_fields.get(field_name)
        if field_def:
            return field_def.get('type') in ('one2many', 'many2many')
        return False

    def _validate_field_node(node):
        """
        Validate and extract field name.
        For column_invisible: only parent.xxx allowed.
        For other modifiers: only direct field names allowed.
        """
        if isinstance(node, ast.Name):
            if modifier == 'column_invisible':
                raise ValidationError(
                    "column_invisible only supports 'parent.field' syntax.\n"
                    "Got: '%s'\n"
                    "Use: column_invisible=\"parent.%s\"" % (node.id, node.id)
                )
            return node.id
        ...
        if isinstance(node, ast.Attribute):
            if isinstance(node.value, ast.Name) and node.value.id == 'parent':
                if modifier == 'column_invisible':
                    return 'parent.%s' % node.attr
                else:
                    raise ValidationError(
                        "'parent.field' syntax is only allowed for column_invisible modifier.\n"
                        "Got: 'parent.%s' in '%s' modifier." % (node.attr, modifier)
                    )
            raise ValidationError(
                "Invalid attribute access. Only 'parent.field' is supported for column_invisible."
            )

        raise ValidationError("Expected field name, got: %s" % ast.dump(node))

    def walk(node, negate=False):
        # Name: invisible="active" → [('active', '!=', False)]
        if isinstance(node, ast.Name):
            field_name = _validate_field_node(node)

            if _is_x2many_field(field_name):
                # x2many field: use [] for empty check
                if negate:
                    # invisible="not one2many_ids" → field is empty
                    return [(field_name, '=', [])]
                else:
                    # invisible="one2many_ids" → field is not empty
                    return [(field_name, '!=', [])]
            else:
                if negate:
        ...
        # Attribute: column_invisible="parent.block_id" → [('parent.block_id', '!=', False)]
        if isinstance(node, ast.Attribute):
            field_name = _validate_field_node(node)
            if field_name:
                if negate:
        ...
        # UnaryOp (not): invisible="not active"
        if isinstance(node, ast.UnaryOp) and isinstance(node.op, ast.Not):
            return walk(node.operand, negate=not negate)

        # BoolOp: and/or
        if isinstance(node, ast.BoolOp):
            subdomains = [walk(v, negate) for v in node.values]

            # Determine operator (De Morgan's law when negated)
            ...
            # Polish notation: need (n-1) operators for n operands
            # Example: a AND b AND c → ['&', '&', a, b, c]
            # Example: a OR b OR c → ['|', '|', a, b, c]
            operator = '&' if is_and else '|'

            result = [operator] * (len(subdomains) - 1)
            for subdomain in subdomains:
                result.extend(subdomain)

            return result

        # Compare: invisible="state == 'draft'"
        if isinstance(node, ast.Compare):
            if len(node.ops) > 1:
                raise ValidationError("Chained comparisons not supported: %s" % expr)

            left = node.left
            op_node = node.ops[0]
            comparator = node.comparators[0]

            # Handle: 42 in one2many_ids (resolved from %(module.record)d in one2many_ids)
            if isinstance(op_node, (ast.In, ast.NotIn)):
                is_in = isinstance(op_node, ast.In)
                if negate:
                    is_in = not is_in

                # Case: 42 in field_name (number/value in x2many field)
                # Case: 42 in parent.field_name
                if isinstance(comparator, (ast.Name, ast.Attribute)):
                    field_name = _validate_field_node(comparator)
                    value = _ast_to_value(left)
                    op = 'in' if is_in else 'not in'
                    if not isinstance(value, (list, tuple)):
                        value = [value]
                    return [(field_name, op, value)]

                # Case: field_name in (values)
                # parent.field_name in (values)
                if isinstance(left, (ast.Name, ast.Attribute)) and isinstance(comparator, (ast.Tuple, ast.List)):
                    field = _validate_field_node(left)
                    values = [_ast_to_value(elt) for elt in comparator.elts]
                    op = 'in' if is_in else 'not in'
                    return [(field, op, values)]

                raise ValidationError("Invalid 'in' expression: %s" % expr)

            if not isinstance(left, (ast.Name, ast.Attribute)):
                raise ValidationError("Left side must be field name: %s" % expr)

            field = _validate_field_node(left)
            op = _OP_MAP.get(type(node.ops[0]))
            if not op:
                raise ValidationError("Unsupported operator: %s" % expr)

            value = _ast_to_value(node.comparators[0])

            if negate and op in _OP_INVERSE:
                op = _OP_INVERSE[op]

            return [(field, op, value)]

        raise ValidationError("Cannot convert expression to domain: %s" % expr)
...
def transfer_node_to_modifiers(node, modifiers, context=None, in_tree_view=False, model_fields=None):
    """
    Transfer node attributes to modifiers dict.

    Supports:
    - New expression-based modifiers: invisible="state == 'draft'"
    - Legacy attrs (with warning): attrs="{'invisible': [('state', '=', 'draft')]}"
    - Legacy states (with warning): states="draft,confirm"
    """

    has_attrs = bool(node.get('attrs'))
    has_states = bool(node.get('states'))
    has_new_modifiers = any(node.get(attr) for attr in VIEW_MODIFIERS)

    tag = node.tag
    name = node.get('name', '')
    element_desc = '<%s name="%s">' % (tag, name) if name else '<%s>' % tag

    # ============================================
    # Check for mixing old and new syntax
    # ============================================

    if has_attrs and has_new_modifiers:
        attrs_dict = safe_eval(node.get('attrs'))
        suggestions = []

        for attr, domain in attrs_dict.items():
            if attr in VIEW_MODIFIERS:
                suggested_expr = _domain_to_expr(domain)
                suggestions.append('  %s="%s"' % (attr, suggested_expr))

        # Check if any new modifier conflicts with attrs
        conflicting = [attr for attr in VIEW_MODIFIERS
                       if node.get(attr) and attr in attrs_dict]
        conflicting_node = [node.get(attr) for attr in VIEW_MODIFIERS
                       if node.get(attr) and attr in attrs_dict]

        if conflicting:
            _logger.warning(
                "Cannot mix 'attrs' with expression-based modifiers.\n\n"
                "Element: %s\n"
                "Conflicting attributes: %s\n\n"
                "Filled 'attrs' dictionary: %s\n"
                "Filled conflicting attribute: %s\n"
                "Please remove 'attrs' and use only the new syntax.\n"
                "Suggested migration for attrs content:\n%s",
                element_desc,
                ', '.join(conflicting),
                attrs_dict,
                conflicting_node,
                '\n'.join(suggestions) if suggestions else '  (no modifiers in attrs)',
            )

    if has_states and has_new_modifiers:
        states = node.get('states')
        states_list = [s.strip() for s in states.split(',')]

        if len(states_list) == 1:
            suggested_expr = "state == %r" % states_list[0]
        else:
            suggested_expr = "state in %r" % (tuple(states_list),)

        # If there's already an invisible expression, suggest combining
        existing_invisible = node.get('invisible')
        if existing_invisible:
            combined_suggestion = "(%s) and (%s)" % (suggested_expr, existing_invisible)
            _logger.warning(
                "Cannot mix 'states' with expression-based modifiers.\n\n"
                "Element: %s\n"
                "Current states: %s\n"
                "Current invisible: %s\n\n"
                "Please remove 'states' and combine into 'invisible'.\n"
                "Suggested migration:\n"
                "  invisible=\"%s\"\n\n"
                "Or if you want visible only in these states:\n"
                "  invisible=\"state not in %r\"",
                element_desc,
                states,
                existing_invisible,
                combined_suggestion,
            ...
            _logger.warning(
                ...
                "Element: %s\n"
                "Current states: %s\n\n"
                "Please remove 'states' and use 'invisible' instead.\n"
                "Suggested migration (visible only in these states):\n"
                "  invisible=\"state not in %r\"\n\n"
                "Or if you want invisible in these states:\n"
                "  invisible=\"state in %r\"",
                element_desc,
                states,
                tuple(states_list),
                tuple(states_list),
    ...
    # ============================================
    # Handle legacy attrs (still supported)
    # ============================================
    if has_attrs:
        attrs_dict = safe_eval(node.get('attrs'))
        ...
        # if suggestions:
        #     _logger.warning(
        #         "DEPRECATED: Element %s uses 'attrs' which is deprecated.\n"
        #         "Suggested migration:\n  %s",
        #         element_desc,
        #         '\n  '.join(suggestions)
        #     )
    ...
    # ============================================
    # Handle legacy states (still supported)
    # ============================================
    if has_states:
        states = node.get('states')
        states_list = [s.strip() for s in states.split(',')]
        state_domain = [('state', 'not in', states_list)]

        existing = modifiers.get('invisible')
        if isinstance(existing, list):
            # Combine with AND
            modifiers['invisible'] = ['&'] + existing + state_domain
        else:
            modifiers['invisible'] = state_domain

        # if len(states_list) == 1:
        #     suggested = "state not in ('%s',)" % states_list[0]
        # else:
        #     states_tuple = tuple(states_list)
        #     suggested = "state not in %r" % (states_tuple,)
        #
        # _logger.warning(
        #     "DEPRECATED: Element %s uses 'states' which is deprecated.\n"
        #     "Suggested migration: invisible=\"%s\"",
        #     element_desc,
        #     suggested
        # )

    # ============================================
    # Handle new-style modifiers
    # ============================================
    for attr in VIEW_MODIFIERS:
        expr = node.get(attr)
        if not expr:
        ...
        # Static boolean: "1", "0", "True", "False"
        if _is_static_bool(expr):
            if in_tree_view and attr == 'invisible' and node.tag == 'field':
                target_attr = 'column_invisible'
            
            modifiers[target_attr] = expr.strip() in ('1', 'True', 'true')
            continue

        # Context-only expression (evaluate server-side)
        if _is_context_only(expr):
            if in_tree_view and attr == 'invisible' and node.tag == 'field':
                target_attr = 'column_invisible'

            modifiers[target_attr] = _eval_context_expr(expr, context)
            continue

        # Field expression (convert to domain)
        try:
            domain = _expr_to_domain(expr, model_fields, modifier=attr)
            modifiers[target_attr] = domain
            ...
            raise ValidationError(
                ...
                "Error: %s\n\n"
                "Supported syntax:\n"
                "  - Boolean field: invisible=\"active\"\n"
                "  - Negation: invisible=\"not active\"\n"
                "  - Comparison: invisible=\"state == 'draft'\"\n"
                "  - In list: invisible=\"state in ('draft', 'confirm')\"\n"
                "  - Combined: invisible=\"state == 'draft' and not active\"\n"
                "  - Context: invisible=\"context.get('hide_field')\"\n"
                "  - Parent field (column_invisible only): column_invisible=\"parent.block_id\"" % (
                    attr, element_desc, expr, e.name
                )
            )


def simplify_modifiers(modifiers):
    for a in VIEW_MODIFIERS:
...
* if False, the view currently does not extend its parent but can be enabled
    ...
    def postprocess(self, model, node, view_id, in_tree_view, model_fields):
        ...
        # The view architecture overrides the python model.
        # Get the attrs before they are (possibly) deleted by check_group below
        transfer_node_to_modifiers(node, modifiers, self._context, in_tree_view, model_fields)
