...
from tigernix import api, fields, models, tools, SUPERUSER_ID, _
from tigernix.exceptions import AccessDenied, AccessError, UserError, ValidationError
from tigernix.http import request
from tigernix.osv import expression
from tigernix.service.db import check_super
from tigernix.tools import partition, collections, lazy_property
...
class Users(models.Model):
    """ User class. A res.users record models an TigernixERP user and is different
        from an employee.
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by TigernixERP (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
        self._invalidate_session_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: tigernix.exceptions.AccessDenied when old password is wrong
        :raise: tigernix.exceptions.UserError when new password is not set or empty
        """
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warn(
                    ...
                    "and *might* be a proxy. If your TigernixERP is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "TigernixERP in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.tigernix.com/documentation/13.0/setup/deploy.html#https for details.",
                    source
                )
    ...
    def _on_login_cooldown(self, failures, previous):
        ...
        min_failures = int(cfg.get_param('base.login_cooldown_after', 50))
        if min_failures == 0:
            return False
    ...
    def _mfa_url(self):
        """ Used for 2FA purpose (if installed) - Added by Kelvyn please contact me if anything wrong happened"""
        return
