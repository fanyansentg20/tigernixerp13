...
from tigernix import tools
from tigernix.addons.base.models.res_partner import _tz_get
from tigernix.osv import expression
from tigernix.tools.translate import _
from tigernix.tools.misc import get_lang
from tigernix.tools import DEFAULT_SERVER_DATE_FORMAT, DEFAULT_SERVER_DATETIME_FORMAT, pycompat
...
class Meeting(models.Model):
    ...
    def _inverse_dates(self):
        for meeting in self:
            if meeting.allday:
                ...
                meeting.write({'start': meeting.start_datetime or meeting.start,
                               'stop': meeting.stop_datetime or meeting.stop})
        ...
        for meeting in self:
            ...
            if meeting.alarm_ids:
                for alarm in meeting.alarm_ids:
                    ...
                    trigger.value = delta
                    valarm.add('DESCRIPTION').value = alarm.name or u'TigernixERP'
    ...
    def _rrule_parse(self, rule_str, data, date_start):
        ...
        #repeat yearly but for tigernix it's monthly, take same information as monthly but interval is 12 times
        if rule._bymonth:
    ...
    def write(self, values):
        #if 'stop_datetime' in values and 'start_datetime' not in values:
        #    values['start_datetime'] = self[0].start_datetime or self[0].start
