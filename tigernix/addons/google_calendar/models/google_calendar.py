...
from tigernix.tools import exception_to_unicode
...
class SyncEvent(object):
    ...
    def compute_OP(self, modeFull=True):
        #If event are already in Gmail and in TigernixERP
        ...
        # New in TigernixERP...  Create on create_events of synchronize function
        elif self.OE.found and not self.GG.found:
            if self.OE.status:
                self.OP = Delete('OE', 'Update or delete from GOOGLE')
            else:
                if not modeFull:
                    self.OP = Delete('GG', 'Deleted from TigernixERP, need to delete it from Gmail if already created')
                else:
                    self.OP = NothingToDo("", "Already Deleted in gmail and unlinked in TigernixERP")
...
class GoogleCalendar(models.AbstractModel):
    ...
    def create_an_event(self, event):
        """ Create a new event in google calendar from the given event in TigernixERP.
    ...
    def update_from_google(self, event, single_event_dict, type):
        """ Update an event in TigernixERP with information from google calendar
    ...
    def update_events(self, lastSync=False):
        ...
        if lastSync:
            ...
            my_tigernix_attendees = CalendarAttendee.with_context(context_novirtual).search([
                ('partner_id', '=', my_partner_id),
                ('event_id.oe_update_date', '>', lastSync and fields.Datetime.to_string(lastSync) or self.get_minTime().fields.Datetime.to_string()),
                ('google_internal_event_id', '!=', False),
            ])

            my_tigernix_googleinternal_records = my_tigernix_attendees.read(['google_internal_event_id', 'event_id'])

            if self.get_print_log():
                _logger.info("Calendar Synchro -  \n\nUPDATE IN GOOGLE\n%s\n\nRETRIEVE FROM OE\n%s\n\nUPDATE IN OE\n%s\n\nRETRIEVE FROM GG\n%s\n\n", all_event_from_google, my_google_att_ids, my_tigernix_attendees.ids, my_tigernix_googleinternal_records)

            for gi_record in my_tigernix_googleinternal_records:
            ...
            my_attendees = (my_google_attendees | my_tigernix_attendees)
            ...
            # Select all events from TigernixERP which have been already synchronized in gmail
    ...
    def authorize_google_uri(self, from_url='http://www.tigernix.com'):
