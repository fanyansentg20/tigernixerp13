...
import tigernix
from tigernix import api, models, registry, exceptions
from tigernix.addons.base.models.ir_http import RequestUID, ModelConverter
from tigernix.addons.base.models.qweb import QWebException
from tigernix.http import request
from tigernix.osv import expression
from tigernix.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if tigernix._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            tigernix._geoip_resolver = GeoIPResolver.open(geofile) or False
        except Exception as e:
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if tigernix._geoip_resolver and request.httprequest.remote_addr:
                record = tigernix._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _dispatch(cls):
        ...
        if request.is_frontend:
            ...
            if request.routing_iteration == 1:
                ...
                if wrong_url_lg or (missing_url_lg and not is_a_bot):
                    ...
                    redirect.set_cookie('frontend_lang', request.lang.code, httponly=True, secure=True)
                    return redirect
                elif url_lg:
                    request.uid = None
                    path.pop(1)
        ...
        if request.is_frontend and cook_lang != request.lang.code and hasattr(result, 'set_cookie'):
            result.set_cookie('frontend_lang', request.lang.code, httponly=True, secure=True)
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except tigernix.exceptions.MissingError:
