...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a TigernixERP DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of tigernix DB
        2 - token. This token will be compared to the token of TigernixERP. He have 1 hour lifetime
        """
...
class Manager(Thread):
    ...
    def load_drivers(self):
        """
        This method loads local files: 'tigernix/addons/hw_drivers/drivers'
    ...
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to TigernixERP database
        """
        server = helpers.get_tigernix_server_url()
        if server:
            subject = helpers.read_file_first_line('tigernix-subject.conf')
            if subject:
            ...
            _logger.warning('TigernixERP server not set')
    ...
    def run(self):
        """
        Thread that will check connected/disconnected device, load drivers if needed and contact the tigernix server with the updates
        """
        helpers.check_certificate()
        updated_devices = {}
