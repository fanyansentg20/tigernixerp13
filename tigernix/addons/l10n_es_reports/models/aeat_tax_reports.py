...
from tigernix import fields, models, api, _
from tigernix.addons.account_reports.models.account_financial_report import FormulaLine
from tigernix.exceptions import UserError
import tigernix.release
from tigernix.tools.float_utils import float_split_str
...
class AEATAccountFinancialReport(models.Model):
    ...
    def _generate_111_115_common_header(self, options, current_company, period, year):
        ...
        tigernix_version = tigernix.release.version.split('.')
        rslt += self._boe_format_string(str(tigernix_version[0]) + str(tigernix_version[1]), length=4)
    ...
    def _mod347_write_type2_partner_record(self, report_data, year, current_company, operation_key, manual_parameters_map, insurance=False, local_negocio=False):
        ...
        rslt += self._boe_format_string(' ') # Not supported by TigernixERP; according to the partners, too few people need this option
