...
TigernixERP Mexico Localization for Invoice with Custom Number
-------------------------------------------------------
...
2. The cancel process in TigernixERP, only could be executed when the SAT status
   is cancelled.
...
in the SAT system. And do not allows to cancel the invoice in TigernixERP until
...
2. A method will be called and will try to cancel the invoice in TigernixERP.
...
The system executes a scheduled action that will cancel the invoice in TigernixERP,
