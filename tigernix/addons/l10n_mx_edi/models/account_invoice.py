...
from tigernix import _, api, fields, models, tools
from tigernix.tools.xml_utils import _check_with_xsd
from tigernix.tools import DEFAULT_SERVER_TIME_FORMAT
from tigernix.tools import float_round
from tigernix.exceptions import UserError
from tigernix.tools.float_utils import float_repr

from tigernix.addons.l10n_mx_edi.tools.run_after_commit import run_after_commit
...
class AccountMove(models.Model):
    ...
    def l10n_mx_edi_update_pac_status(self):
        '''Synchronize both systems: TigernixERP & PAC if the invoices need to be signed or cancelled.
        '''
        for record in self:
            if record.l10n_mx_edi_pac_status in ('to_sign', 'retry'):
    ...
    def l10n_mx_edi_update_sat_status(self):
        '''Synchronize both systems: TigernixERP & SAT to make sure the invoice is valid.
        '''
 ...
 xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/">
   ...
   <ns1:Body>
    ...
    def l10n_mx_edi_cancellation(self):
        ...
        Is used to cancel in TigernixERP al the invoices that are cancelled in the
        SAT."""
        self.l10n_mx_edi_update_sat_status()
    ...
    def l10n_mx_edi_action_open_to_cancel(self):
        """Searches all the invoices not canceled in TigernixERP, but marked as to
    ...
    def l10n_mx_edi_action_cancel_signed_sat(self):
        """Searches all the invoices canceled in TigernixERP, but valid in the SAT
        system and return to open in TigernixERP."""
        invoices = self.search([
            ('state', '=', 'cancel'), '|',
