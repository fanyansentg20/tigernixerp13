...
from tigernix.addons.l10n_mx_edi.tests.common import InvoiceTransactionCase
...
class TestL10nMxEdiCancelTest(InvoiceTransactionCase):
    ...
    def test_case2(self):
        """The cron that cancel in TigernixERP when the PAC status is to_cancel is
        executed"""
        invoice = self.create_invoice()
        invoice.sudo().journal_id.update_posted = True
        invoice.post()
    ...
    def test_case3(self):
        """The cron that cancel in TigernixERP when the SAT status is cancelled is
        executed"""
        invoice = self.create_invoice()
        invoice.sudo().journal_id.update_posted = True
        invoice.post()
