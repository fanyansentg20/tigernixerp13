...
from tigernix import fields as tigernix_fields, http, tools, _, SUPERUSER_ID
from tigernix.exceptions import ValidationError, AccessError, MissingError, UserError
from tigernix.http import content_disposition, Controller, request, route
from tigernix.tools import consteq
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': tigernix_fields.Date.to_string(tigernix_fields.Date.from_string(date_begin)),
                'date_end': tigernix_fields.Date.to_string(tigernix_fields.Date.from_string(date_end)),
                'name': label,
