...
class TestSaleToInvoice(TestCommonSaleNoChart):
    ...
    def test_refund_create(self):
        ...
        credit_note_wizard = self.env['account.move.reversal'].with_context({'active_ids': [self.invoice.id], 'active_id': self.invoice.id, 'active_model': 'account.move'}).create({
            'refund_method': 'refund',  # this is the only mode for which the SO line is linked to the refund (https://github.com/tigernix/tigernix/commit/e680f29560ac20133c7af0c6364c6ef494662eac)
    ...
    def test_refund_modify(self):
        ...
        # Make a credit note
        credit_note_wizard = self.env['account.move.reversal'].with_context({'active_ids': [self.invoice.id], 'active_id': self.invoice.id, 'active_model': 'account.move'}).create({
            'refund_method': 'modify',  # this is the only mode for which the SO line is linked to the refund (https://github.com/tigernix/tigernix/commit/e680f29560ac20133c7af0c6364c6ef494662eac)
