...
from tigernix.addons.sale_ebay.tools.ebaysdk import Trading
from xml.sax.saxutils import escape

from tigernix import models, fields, api, _
from tigernix.exceptions import UserError, RedirectWarning, ValidationError
        ...
        if self.attribute_line_ids:
            for attribute in self.attribute_line_ids:
                if len(attribute.value_ids) == 1:
                    ...
                    # We used the attributes in TigernixERP to match the Brand and MPN attributes
    ...
    def _sync_available_qty(self):
        if self.ebay_sync_stock:
            if self.ebay_listing_status in ['Active', 'Error']:
                ...
                # Check if the quantity in TigernixERP is different than the one on eBay
                ...
                # The product is Out Of Stock on eBay but there is stock in TigernixERP
