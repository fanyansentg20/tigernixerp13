...
from tigernix import exceptions
from tigernix.addons.test_mail.data import test_mail_data
from tigernix.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE
from tigernix.addons.test_mail.models.test_mail_models import MailTestGateway
from tigernix.addons.test_mail.tests.common import BaseFunctionalTest, MockEmails
from tigernix.addons.test_mail.tests.common import mail_new_test_user
from tigernix.tests import tagged
from tigernix.tools import email_split_and_format, mute_logger, pycompat, formataddr
...
class TestMailgateway(BaseFunctionalTest, MockEmails):
    ...
    def setUpClass(cls):
        ...
        cls.fake_email = cls.env['mail.message'].create({
            ...
            'message_id': '<123456-tigernixerp-%s-mail.test.gateway@%s>' % (cls.test_record.id, socket.gethostname()),
        })
    ...
    @mute_logger('tigernix.addons.mail.models.mail_thread', 'tigernix.models')
    ...
    @mute_logger('tigernix.addons.mail.models.mail_thread')
    ...
    @mute_logger('tigernix.addons.mail.models.mail_thread', 'tigernix.models.unlink', 'tigernix.addons.mail.models.mail_mail')
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by tigernix should update thread accordingly """
    ...
    @mute_logger('tigernix.addons.mail.models.mail_thread', 'tigernix.models', 'tigernix.addons.mail.models.mail_mail')
    ...
    def test_message_process_extra_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since TigernixERP saas-3 """
        self.assertRaises(ValueError,
                          self.format_and_process, MAIL_TEMPLATE,
                          self.partner_1.email_formatted, 'noone@test.com', subject='spam',
                          extra='In-Reply-To: <12321321-tigernixerp-%d-mail.test.gateway@%s>' % (self.test_record.id, socket.gethostname()))

        # when 6.1 messages are present, compat mode is available
        # TigernixERP 10 update: compat mode has been removed and should not work anymore
