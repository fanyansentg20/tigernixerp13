...
from tigernix.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE_PLAINTEXT
from tigernix.addons.test_mail.models.test_mail_models import MailTestSimple
from tigernix.exceptions import AccessError
from tigernix.tools import mute_logger, formataddr
...
class TestMessagePost(BaseFunctionalTest, TestRecipients, MockEmails):
    ...
    def test_post_inactive_follower(self):
        # In some case tigernixbot is follower of a record.
        # Even if it shouldn't be the case, we want to be sure that tigernixbot is not notified
        (self.user_employee | self.user_admin).write({'notification_type': 'inbox'})
    ...
    def test_post_answer(self):
        ...
        self.assertEmails(self.user_employee.partner_id, [[self.partner_1]], ref_content='tigernixerp-%d-mail.test.simple' % self.test_record.id)
        # self.assertTrue(all('tigernixerp-%d-mail.test.simple' % self.test_record.id in m['references'] for m in self._mails))
    ...
    @mute_logger('tigernix.addons.mail.models.mail_mail', 'tigernix.addons.mail.models.mail_thread')
