...
from tigernix.addons.sms.models.sms_sms import SmsSms as SmsSms
from tigernix.addons.sms.tests import common as sms_common
from tigernix.addons.test_mail_full.tests import common as test_mail_full_common
...
class LinkTrackerMock(common.BaseCase):
    ...
    def setUp(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('web.base.url', 'https://test.tigernix.com')

        link_tracker_favicon_patch = patch('tigernix.addons.link_tracker.models.link_tracker.LinkTracker._compute_favicon', wraps=_compute_favicon)
        link_tracker_title_patch = patch('tigernix.addons.link_tracker.models.link_tracker.LinkTracker._get_title_from_url', wraps=_get_title_from_url)
...
class TestSMSPost(test_mail_full_common.BaseFunctionalTest, sms_common.MockSMS, LinkTrackerMock):
    ...
    def test_body_link_shorten(self):
        ...
        link = 'https://test.tigernix.com/my/super_page?test[0]=42&toto=áâà#title3'
        self.env['link.tracker'].search([('url', '=', link)]).unlink()
        new_body = self.env['link.tracker']._convert_links_text('Welcome to %s !' % link, self.tracker_values)
        self.assertNotIn(link, new_body)
    ...
    def test_body_link_shorten_wshort(self):
        link = 'https://test.tigernix.com/r/RAOUL'
        self.env['link.tracker'].search([('url', '=', link)]).unlink()
        new_body = self.env['link.tracker']._convert_links_text('Welcome to %s !' % link, self.tracker_values)
    ...
    def test_body_link_shorten_wunsubscribe(self):
        link = 'https://test.tigernix.com/sms/3/'
        self.env['link.tracker'].search([('url', '=', link)]).unlink()
        new_body = self.env['link.tracker']._convert_links_text('Welcome to %s !' % link, self.tracker_values)
        self.assertIn(link, new_body)
        self.assertFalse(self.env['link.tracker'].search([('url', '=', link)]))
    ...
    def test_sms_body_link_shorten_suffix(self):
        ...
        sms_0 = self.env['sms.sms'].create({
            'body': 'Welcome to https://test.tigernix.com',
            'number': '12',
            'mailing_id': mailing.id,
        })
        sms_1 = self.env['sms.sms'].create({
            'body': 'Welcome to https://test.tigernix.com/r/RAOUL',
            'number': '12',
        })
        ...
        sms_3 = self.env['sms.sms'].create({
            'body': 'Welcome to https://test.tigernix.com/leodagan/r/RAOUL',
            'number': '12', 'mailing_id': mailing.id,
        })

        res = (sms_0 | sms_1 | sms_2 | sms_3)._update_body_short_links()
        self.assertEqual(res[sms_0.id], 'Welcome to https://test.tigernix.com')
        self.assertEqual(res[sms_1.id], 'Welcome to https://test.tigernix.com/r/RAOUL')
        self.assertEqual(res[sms_2.id], 'Welcome to https://test.tigernix.com/r/RAOUL/s/%s' % sms_2.id)
        self.assertEqual(res[sms_3.id], 'Welcome to https://test.tigernix.com/leodagan/r/RAOUL')
