...
import tigernix
import tigernix.modules.registry
from tigernix.api import call_kw, Environment
from tigernix.modules import get_module_path, get_resource_path
from tigernix.tools import image_process, topological_sort, html_escape, pycompat, ustr, apply_inheritance_specs, lazy_property
from tigernix.tools.mimetypes import guess_mimetype
from tigernix.tools.translate import _
from tigernix.tools.misc import str2bool, xlsxwriter, file_open
from tigernix.tools.safe_eval import safe_eval
from tigernix import http, tools
from tigernix.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception, Response
from tigernix.exceptions import AccessError, UserError, AccessDenied
from tigernix.models import check_method_name
from tigernix.service import db, security
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('tigernix.addons.web', "views")
...
# Part of db path configuration
db_paths_list_string = tools.config.get('db_paths')
DB_URLS = ['/web/database/path/selector']
if db_paths_list_string:
    db_paths_list_pairs = [item.split(':') for item in db_paths_list_string.split(',')]
    DB_URLS += ['/%s' % pair[0] for pair in db_paths_list_pairs] + ['/%s/<path:path>' % pair[0] for pair in db_paths_list_pairs]
...
def module_installed_bypass_session(dbname):
    try:
        registry = tigernix.registry(dbname)
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, tigernix.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = tigernix.conf.server_wide_modules or []
    serverside = ['base', 'web']
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("tigernix.addons.web.main.manifest_list(): debug parameter is deprecated")
    mods = mods.split(',')
...
def fix_view_modes(action):
    """ For historical reasons, TigernixERP has weird dealings in relation to
...
def _local_web_translations(trans_file):
    ...
    try:
        ...
        return
    for x in po:
        if x.id and x.string and "tigernixerp-web" in x.auto_comments:
...
#----------------------------------------------------------
# TigernixERP Web web Controllers
#----------------------------------------------------------
class Home(http.Controller):
    ...
    @http.route(['/web/login', '/<db_selector>/web/login'], type='http', auth="none", cors="*")
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = tigernix.SUPERUSER_ID

        values = request.params.copy()
        try:
            values['databases'] = http.db_list()
        except tigernix.exceptions.AccessDenied:
        ...
        if request.httprequest.method == 'POST':
            ...
            try:
                ...
                redirect_response = http.redirect_with_hash(
                ...
                if origin:
                    redirect_response.headers["Access-Control-Allow-Origin"] = origin
            ...
            except tigernix.exceptions.AccessDenied as e:
                request.uid = old_uid
                if e.args == tigernix.exceptions.AccessDenied().args:
        ...
        print("origin", request.httprequest.headers.get('Origin'))
        response = request.render('web.login', values)
        response.headers['X-Frame-Options'] = 'DENY'

        origin = request.httprequest.headers.get('Origin')
        if origin and request.httprequest.method == 'GET':
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = tigernix.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def version_info(self):
        return tigernix.service.common.exp_version()
...
class Proxy(http.Controller):
    ...
    def post(self, path):
        ...
        with request.session.load_request_data() as data:
            ...
            headers = {'X-Tigernixerp-Session-Id': request.session.sid}
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = tigernix.tools.config.verify_admin_password('admin')
        d['list_db'] = tigernix.tools.config['list_db']
        d['langs'] = tigernix.service.db.exp_list_lang()
        d['countries'] = tigernix.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = tigernix.service.db.list_db_incompatible(d['databases'])
        except tigernix.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    # Part of db path configuration
    @http.route(DB_URLS, type='http', auth="none", csrf=False)
    def db_path_selection(self, path=None, **kw):
        _logger.info(path)
        query_string = request.httprequest.query_string.decode("utf-8")
        if path:
            params = "?" + query_string if query_string else ""
            redirect_url = "/" + path + params
        else:
            redirect_url = '/'
    ...
    def duplicate(self, master_pwd, name, new_name, neutralize_database=True):
        try:
    ...
    def backup(self, master_pwd, name, backup_format = 'zip', filestore=True):
        ...
        try:
            tigernix.service.db.check_super(master_pwd)
            ...
            dump_stream = tigernix.service.db.dump_db(name, None, backup_format, filestore)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=tigernix.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.tigernix.com/oauth2/auth?' + werkzeug.url_encode(params)
    ...
    def _content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        if not image_base64:
            ...
            if not (width or height):
                width, height = tigernix.tools.image_guess_size_from_field_name(field)
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = tigernix.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: tigernix.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from TigernixERP's export data to whatever the
...
class CSVExport(ExportFormat, http.Controller):
    ...
    def from_group_data(self, fields, groups):
        ...
        def write_child_row(writer,groups,group_depth=0):
            for group_name, group in groups.children.items():
                group_name = group_name[1] if isinstance(group_name, tuple) and len(group_name) > 1 else group_name
                rows = group.data
                row = []
                label = '%s%s (%s)' % ('    ' * group_depth, group_name, group.count)
                row.append(pycompat.to_text(label))
                writer.writerow(row)
                if group.children:
                    write_child_row(writer, group, group_depth=group_depth+1)
...
class ExcelExport(ExportFormat, http.Controller):
    ...
    def index(self, data, token):
        # return self.base(data, token)
        # Modified on 10/09/2024 to implement message validation, raising a warning when exporting data.
        # Error detail: An exception occured during an http request
        try:
            return self.base(data, token)
        except Exception as e:
            se = _serialize_exception(e)
...
class Base64Controller(http.Controller):
    @http.route('/base64/image', type='http', auth="none")
    def image_base64(self, **kwargs):
        image_bytes = base64.b64decode(kwargs.get('string'))
