...
Client actions are the client-side version of TigernixERP's "Server
...
In the TigernixERP Web code, a client action only requires two pieces of
information:
...
* Providing said object. Two different types of objects can be mapped
  ...
  * An TigernixERP Web widget, which must inherit from
    :js:class:`tigernixerp.web.Widget`
...
* if, on the other hand, the client action maps to a
  :js:class:`~tigernixerp.web.Widget`, that
  :js:class:`~tigernixerp.web.Widget` will be instantiated and added to
  the web client's canvas, with the usual
  :js:class:`~tigernixerp.web.Widget` lifecycle (essentially, it will
...
object::
    ...
    // Registers the object 'tigernixerp.web_dashboard.Widget' to the client
...
At this point, the generic :js:class:`~tigernixerp.web.Widget` lifecycle
...
should be performed in the :js:func:`~tigernixerp.web.Widget.start()`
method.

.. note::

    As required by :js:class:`~tigernixerp.web.Widget`'s contract, if
    :js:func:`~tigernixerp.web.Widget.start()` executes any asynchronous
