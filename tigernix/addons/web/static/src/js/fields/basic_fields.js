tigernix.define('web.basic_fields', function (require) {
"use strict";
...
var InputField = DebouncedField.extend({
     ...
     *
     * @param {TigernixEvent} event 'field_changed' event
     */
...
var FieldDateRange = InputField.extend({
    ...
    _applyChanges: function (ev, picker) {
        ...
        if (this.isDateField) {
            ...
            // time at 00:00:00. So, TigernixERP will consider it as UTC. To fix this added browser
...
var JournalDashboardGraph = AbstractField.extend({
    ...
    _getLineChartConfig: function () {
        ...
        var borderColor = this.data[0].is_sample_data ? '#dddddd' : '#f3a32a';
        var backgroundColor = this.data[0].is_sample_data ? '#ebebeb' : '#f7cd8d';
        return {
            type: 'line',
            data: {
                labels: labels,
    ...
    _getBarChartConfig: function () {
        ...
        this.data[0].values.forEach(function (pt) {
            ...
            var color = pt.type === 'past' ? '#f7cd8d' : (pt.type === 'future' ? '#99D6FF' : '#ebebeb');
