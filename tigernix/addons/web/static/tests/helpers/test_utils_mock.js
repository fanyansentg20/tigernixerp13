tigernix.define('web.test_utils_mock', function (require) {
"use strict";
 ...
 *   used to generate mock answers for every kind of routes supported by TigernixERP
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught TigernixERP
 *   events.
...
function addMockEnvironment(widget, params) {
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            tigernix.debug = params.config.debug;
        }
    }
    ...
    // make sure all TigernixERP events bubbling up are intercepted
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any TigernixERP widget)
