...
from tigernix.addons.crm.tests.common import TestCrmCases
...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('tigernix.addons.base_geolocalize.models.base_geocoder.GeoCoder.geo_find', wraps=geo_find)
        patcher.start()
...
class TestPartnerLeadPortal(TestCrmCases):
    ...
    def setUp(self):
        ...
        self.portal_user = self.env['res.users'].with_context({'no_reset_password': True, 'mail_notrack': True}).create({
            'name': 'Super Customer TigernixERP Intregrating Partner',
