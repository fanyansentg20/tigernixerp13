...
"""
TigernixERP - Server
TigernixERP is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Tigernix - TigernixERP
"""
...
__author__ = tigernix.release.author
__version__ = tigernix.release.version

# Also use the `tigernix` logger for the main script.
_logger = logging.getLogger('tigernix')
...
def report_configuration():
    ...
    """
    config = tigernix.tools.config
    _logger.info("TigernixERP version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', tigernix.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = tigernix.tools.config
    if not tigernix.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = tigernix.modules.registry.Registry.new(dbname)
        with tigernix.api.Environment.manage():
            with registry.cursor() as cr:
                tigernix.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with tigernix.api.Environment.manage():
        with registry.cursor() as cr:
            tigernix.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    tigernix.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                tigernix.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except tigernix.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        tigernix.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = tigernix.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the tigernix server (default command)"""
    def run(self, args):
        main(args)
