...
# *broken*
NO_POSTMORTEM = (tigernix.exceptions.except_orm,
                 tigernix.exceptions.AccessDenied,
                 tigernix.exceptions.Warning,
                 tigernix.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                tigernix.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = tigernix.service.common.dispatch
        elif service_name == 'db':
            dispatch = tigernix.service.db.dispatch
        elif service_name == 'object':
            dispatch = tigernix.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                tigernix.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                tigernix.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except tigernix.exceptions.DeferredException as e:
        _logger.exception(tigernix.tools.exception_to_unicode(e))
        tigernix.tools.debugger.post_mortem(tigernix.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(tigernix.tools.exception_to_unicode(e))
        tigernix.tools.debugger.post_mortem(tigernix.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in tigernix.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~tigernix.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~tigernix.api.Environment` bound to current request. """
        if self._env is None:
            self._env = tigernix.api.Environment(self.cr, self.uid, self.context)
        return self._env

    @lazy_property
    def session(self):
        """ :class:`TigernixERPSession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            tigernix.tools.debugger.post_mortem(
                tigernix.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return tigernix.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <tigernix.http.WebRequest.csrf_token`, the
          :data:`~tigernix.http.request` object is available by default
...
class JsonRequest(WebRequest):
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def __init__(self, *args):
        ...
        try:
            self.jsonrequest = json.loads(request or '{}')
        except ValueError:
    ...
    def _json_response(self, result=None, error=None):
        ...
        if result is not None:
            if isinstance(result, dict):
                default_code = result.pop('json_status_code_custom', default_code)
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (tigernix.exceptions.Warning, tigernix.exceptions.except_orm,
...
def serialize_exception(e):
    ...
    if isinstance(e, tigernix.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, tigernix.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, tigernix.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, tigernix.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, tigernix.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, tigernix.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, tigernix.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, tigernix.exceptions.except_orm):
...
HTTP methods). See
https://www.tigernix.com/documentation/13.0/reference/http.html#csrf for
more details.
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["tigernix", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('tigernix.addons.') and c.__module__.split(".")[2] in modules
...
class TigernixERPSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(TigernixERPSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = tigernix.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = tigernix.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or tigernix.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = tigernix.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        return werkzeug.contrib.sessions.FilesystemSessionStore(
            path, session_class=TigernixERPSession, renew_missing=True)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        routing_map = werkzeug.routing.Map(strict_slashes=False, converters=None)
        for url, endpoint, routing in tigernix.http._generate_routing_rules([''] + tigernix.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in tigernix.addons.__path__:
    ...
    def setup_session(self, httprequest):
        ...
        if not sid:
            sid =  httprequest.headers.get("X-Tigernixerp-Session-Id")
        if not sid:
    ...
    def get_response(self, httprequest, result, explicit_session):
        ...
        if not explicit_session and hasattr(response, 'set_cookie'):
            max_age = tigernix.tools.config.get('session_max_age', 1 * 8 * 60 * 60) or None
            response.set_cookie(
                'session_id', httprequest.session.sid, max_age=max_age, httponly=True)
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            if tigernix.tools.config.get('prevent_crawling', True):
            ...
            with request:
                ...
                if db:
                    try:
                        tigernix.registry(db).check_signaling()
                        with tigernix.tools.mute_logger('tigernix.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = tigernix.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if tigernix.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = tigernix.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif tigernix.tools.config['db_name']:
        ...
        exposed_dbs = set(db.strip() for db in tigernix.tools.config['db_name'].split(','))
        dbs = sorted(exposed_dbs.intersection(dbs))
    else:
        # How to use db_path by adding the following config in the ERP configuration file
        # db_paths = path1:DATABASE_NAME1,path2:DATABASE_NAME2
        # by adding this, the database can be accessed with domain.com/path1 or domain.com path2
        # This part added by Rangga, if you found any issue please contact me.
        db_paths_list_string = tigernix.tools.config.get('db_paths')
        if db_paths_list_string:
            db_path_list = dict(item.split(':') for item in db_paths_list_string.split(','))
            path_list = httprequest.path.split('/')
            if len(path_list) > 1 and path_list[1] in db_path_list:
                dbs = [db_path_list[path_list[1]]]
...
def db_monodb(httprequest=None):
    ...
    # try the db already in the session and check default db is set.
    db_session = httprequest.session.db or tigernix.tools.config.get('db_default')
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = tigernix.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = tigernix.tools.ustr(filename)
