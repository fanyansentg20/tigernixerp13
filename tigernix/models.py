...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (TigernixERP) module name can be in the ``tigernix.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``tigernix.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['tigernix', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """Base class for TigernixERP models.

    TigernixERP models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~tigernix.models.BaseModel._register` attribute may be set to False.
    """

    _auto = False
    """Whether a database table should be created (default: ``True``).
    If set to ``False``, override :meth:`~tigernix.models.BaseModel.init`
    to create the database table.

    .. tip:: To create a model without any table, inherit
            from :class:`~tigernix.models.AbstractModel`.
    """
    ...
    .. warning::
      ...
      :attr:`~tigernix.models.Model._inherits`-ed models, the inherited field will
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = tigernix.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or tigernix.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from tigernix.fields import Char, Integer
    ...
    def _fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        if view_id:
            ...
            if self._name == root_view['model']:
                get_default_view = False
...
""")
    ...
    def unlink(self):
        ...
        from tigernix.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~tigernix.fields.Integer`,
          :class:`~tigernix.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~tigernix.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~tigernix.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~tigernix.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~tigernix.fields.Date` and
              :class:`~tigernix.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`tigernix.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`tigernix.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _tigernixerp/models/relationals/format:

          :class:`~tigernix.fields.One2many` and
          :class:`~tigernix.fields.Many2many` use a special "commands" format to
        ...
        for fname in vals:
            ...
            #     length = len(field.related)
            #     parent_field = field.related[0]
            #     parent_fname = field.related[-1]
            #     parent_frelated = self[parent_field]

            #     if length > 2:   
            #         i = 1
            #         while i != length-1:
            #             parent_field = field.related[i]
            #             parent_frelated = parent_frelated[parent_field]                       
            #             i += 1

            #     related_field = field
            #     related_data = {}
            #     while related_field != None:
            #         related_field = parent_frelated._fields.get(parent_fname)
            #         related_data[parent_field] = parent_frelated
            #         # related_field.write(parent_frelated, vals[fname])
            #         if not related_field.related:
            #             break
            #         parent_field = related_field.related[0]
            #         parent_frelated = parent_frelated[parent_field] 
            ...
            #         except:
            #             continue
            #         if not model_obj._transient and not model_obj._abstract:
            #             for kfield in model_obj._fields:
            #                 if model_obj._fields.get(kfield):
            #                     model_field = model_obj._fields[kfield]
            #                     store = model_field.store
            #                     related = model_field.related_field
            #                     for x in related_data.keys():
            #                         if related and parent_fname in model_field.related and x in model_field.related:
            #                             print('Model', model_name, model_field, model_field.related)
                                    # related_vals[model_field] = {}
                                    # related_vals[model_field].update({
                                    #     'model_name':model_name,
                                    #     'related': model_field.related
                                    # })                                  
                                # if store and model_field.type in ('many2one','many2many','one2many') and model_field.comodel_name == 'res.partner':
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an TigernixERP domain.
            ...
            if order_field == 'id':
                ...
                if field.store and field.type == 'many2one':
                    ...
                    if field.type in ['char','text']:
                        qualifield_name += ' COLLATE "C" '
                    # ----
                    order_by_elements.append("%s %s" % (qualifield_name, order_direction))
                elif getattr(field,'custom_order',''):
                    order_by_elements.append("%s %s" % (field.custom_order, order_direction))
                else:
#                    _logger.warning("Model %r cannot be sorted on field %r (not a column)", self._name, order_field)
    ...
    def ensure_one(self):
        ...
        :raise tigernix.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~tigernix.api.Environment`
        ...
        for field, value in sorted(field_values, key=is_monetary):
            ...
            if field.relational:
                ...
                for invf in self._field_inverses[field]:
                    ...
                    # /home/dle/src/tigernix/master-nochange-fp/tigernix/addons/test_new_api/tests/test_new_fields.py
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted TigernixERP models.

    TigernixERP models are created by inheriting from this class::
