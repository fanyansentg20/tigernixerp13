...
from tigernix.tools import (assertion_report, config, existing_tables, ignore,
                        lazy_classproperty, lazy_property, OrderedSet)
from tigernix.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # tigernix.service.wsgi_server.application
        ...
        with cls._lock:
            with tigernix.api.Environment.manage():
                ...
                try:
                    ...
                    try:
                        tigernix.modules.load_modules(registry._db, force_demo, status, update_module)
                    except Exception:
                        tigernix.modules.reset_modules_state(db_name)
                        raise
                except Exception:
    ...
    def init(self, db_name):
        ...
        self._db = tigernix.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            has_unaccent = tigernix.modules.db.has_unaccent(cr)
            if tigernix.tools.config['unaccent'] and not has_unaccent:
                _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
            self.has_unaccent = tigernix.tools.config['unaccent'] and has_unaccent
    ...
    def init_models(self, cr, model_names, context):
        ...
        env = tigernix.api.Environment(cr, SUPERUSER_ID, context)
