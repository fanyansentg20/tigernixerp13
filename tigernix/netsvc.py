...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('tigernix.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    # Check that handler.stream has a fileno() method: when running TigernixERP
...
DEFAULT_LOG_CONFIGURATION = [
    'tigernix.http.rpc.request:INFO',
    'tigernix.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['tigernix:DEBUG', 'tigernix.sql_db:INFO', 'tigernix.http.rpc:DEBUG'],
    'debug_rpc': ['tigernix:DEBUG', 'tigernix.sql_db:INFO', 'tigernix.http.rpc.request:DEBUG'],
    'debug': ['tigernix:DEBUG', 'tigernix.sql_db:INFO'],
    'debug_sql': ['tigernix.sql_db:DEBUG'],
    'info': [],
    'warn': ['tigernix:WARNING', 'werkzeug:WARNING'],
    'error': ['tigernix:ERROR', 'werkzeug:ERROR'],
    'critical': ['tigernix:CRITICAL', 'werkzeug:CRITICAL'],
}
