...
import tigernix
from tigernix.exceptions import UserError, ValidationError, QWebException
from tigernix.models import check_method_name
from tigernix.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = tigernix.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(tigernix.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if tigernix.registry(dbname)._init and not tigernix.tools.config['test_enable']:
                    raise tigernix.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    tigernix.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = tigernix.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return tigernix.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with tigernix.registry(db).cursor() as cr:
