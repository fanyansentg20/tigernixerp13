...
from werkzeug.exceptions import InternalServerError, BadRequest, NotFound, MethodNotAllowed, NotAcceptable, RequestTimeout
...
import tigernix
from tigernix.modules import get_modules
from tigernix.modules.module import run_unit_tests, get_test_modules
from tigernix.modules.registry import Registry
from tigernix.release import nt_service_name
from tigernix.tools import config
from tigernix.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
#Jimmy add code here
def ise_get_body(self, environ=None):
    error_log = traceback.format_exc().replace('odoo','tigernix')
    tz = pytz.timezone('Asia/Singapore')
    datetime_now = datetime.datetime.now().astimezone(tz)
    try:
        ROOT_DIR = os.environ.get('USER')
        ROOT_DIR = '/home/' + ROOT_DIR

        if not os.path.exists(ROOT_DIR):
            ROOT_DIR = '/home/ubuntu'

        log_dir = ROOT_DIR + '/tigernixerp_log/system_log'
        date_val = datetime_now.strftime('%d_%b_%Y')
        file_name = log_dir + '/system_log_' + date_val + '.log'
        ...
        f = open(file_name,"a+")
        f.write(datetime_now.strftime('%d %b %Y %H:%M:%S %Z'))
        f.write('\n')
        f.write(error_log)
        f.write('\n\n')
        f.write("==============================================================================")
    ...
    # error_html = open("./tigernix/addons/v13_tigernix_default/view/internal_server_error.html", "r")
    # Modified at 23082024 to get the directory for file internal_server_error.html to fix the error FileNotFoundError
    file_path = os.path.dirname(os.path.abspath(__file__)) + "/../addons/v13_tigernix_default/view/internal_server_error.html"
    error_html = open(file_path, "r")    
    return text_type(error_html.read())
...
def get_body(self, environ=None):
    ...
    file_path = os.path.dirname(os.path.abspath(__file__)) + "/../addons/v13_tigernix_default/view/internal_server_error.html"
    error_html = open(file_path, "r")    
    error_html = error_html.read().replace("$code", str(self.code))
    error_html = error_html.replace("$title", escape(self.name))
    error_html = error_html.replace("$description", self.get_description(environ))
    return text_type(error_html)

InternalServerError.get_body = ise_get_body
NotFound.get_body = get_body
BadRequest.get_body = get_body
MethodNotAllowed.get_body = get_body
NotAcceptable.get_body = get_body
RequestTimeout.get_body = get_body
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'tigernix.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    # is included in all Python 3 releases installed on all operating systems supported by TigernixERP.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(tigernix, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in tigernix.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="tigernix.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from tigernix.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = tigernix.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(tigernix.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="tigernix.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="tigernix.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(tigernix, 'phoenix', None):
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('tigernix: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(tigernix.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = tigernix.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('tigernix.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from tigernix.addons import base
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(tigernix.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            tigernix.modules.module.load_tigernixerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `tigernixerp-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute tigernixerp-server process with (nearly) the same arguments"""
    if tigernix.tools.osutil.is_running_as_nt_service():
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    ...
                    result = tigernix.modules.module.TigernixTestRunner().run(suite)
        ...
        try:
            ...
            if config['test_file']:
                ...
                if not os.path.isfile(test_file):
                    ...
                    with tigernix.api.Environment.manage():
                        load_test_file_py(registry, test_file)

            # run post-install tests
            if config['test_enable']:
                t0 = time.time()
                t0_sql = tigernix.sql_db.sql_counter
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, tigernix.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the tigernix http server and cron processor.
    """
    global server

    load_server_wide_modules()
    tigernix.service.wsgi_server._patch_xmlrpc_marshaller()

    if tigernix.evented:
        server = GeventServer(tigernix.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(tigernix.service.wsgi_server.application)
        ...
        server = ThreadedServer(tigernix.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not tigernix.evented:
        if inotify:
    ...
    if getattr(tigernix, 'phoenix', False):
        _reexec()
