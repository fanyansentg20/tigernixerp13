...
# User code must use the exceptions defined in ``tigernix.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, tigernix.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, tigernix.tools.ustr(e.name))
    ...
    elif isinstance(e, tigernix.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = tigernix.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, tigernix.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(tigernix.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(tigernix.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, tigernix is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(tigernix.tools.ustr(value), write)
...
def application_unproxied(environ, start_response):
    ...
    # web.session.TigernixERPSession.send() and at RPC dispatch in
    # tigernix.service.web_services.objects_proxy.dispatch().
    ...
    with tigernix.api.Environment.manage():
        result = tigernix.http.root(environ, start_response)
