# -*- coding: utf-8 -*-
"""
The module :mod:`tigernix.tests.common` provides unittest test cases and a few
...
from tigernix.models import BaseModel
from tigernix.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from tigernix.tools import float_compare, single_email_re
from tigernix.tools.misc import find_in_path
...
import tigernix
import pprint
from tigernix import api
from tigernix.service import security



_logger = logging.getLogger(__name__)

# The tigernix library is supposed already configured.
ADDONS_PATH = tigernix.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = tigernix.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = tigernix.SUPERUSER_ID


def get_db_name():
    db = tigernix.tools.config['db_name']
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most tigernix operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('tigernix.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    """
    Subclass of TestCase for common TigernixERP-specific code.

    This class is abstract and expects self.registry, self.cr and self.uid to be
    initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.tigernix.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~tigernix.models.BaseModel`
        """
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/tigernix/addons/" in filename:
                        filename = filename.rsplit("/tigernix/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = tigernix.registry(get_db_name())
        #: current transaction's cursor
        self.cr = self.cursor()
        #: :class:`~tigernix.api.Environment` for the current test case
        self.env = api.Environment(self.cr, tigernix.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.registry = tigernix.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.env = api.Environment(cls.cr, tigernix.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_tigernix')
        self.chrome_process = None

        otc = tigernix.tools.config
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = tigernix.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        tigernix.http.root.session_store.save(self.session)
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('tigernix.service.http.request.')]
        ...
        if request_threads:
            ...
            tigernix.tools.misc.dumpstacks()

    def authenticate(self, user, password):
        # stay non-authenticated
        if user is None:
            if self.session:
                tigernix.http.root.session_store.delete(self.session)
        ...
        # TigernixERPSession.authenticate accesses the current request, which we
        ...
        tigernix.http.root.session_store.save(session)
        if self.browser:
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "tigernix.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "tigernix.__DEBUG__.services['web_tour.tour'].tours.%s.ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~tigernix.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~tigernix.fields.Many2many` fields, the field itself is a
    :class:`~tigernix.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~tigernix.fields.One2many` are reified as
    :class:`~tigernix.tests.common.O2MProxy`.

    Because the :class:`~tigernix.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~tigernix.tests.common.O2MProxy.new` and
    :meth:`~tigernix.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: tigernix.models.Model
    ...
    :type view: int | str | tigernix.model.Model
    ...
    def _init_from_defaults(self, model):
        ...
        def cleanup(k, v):
            if fields[k]['type'] == 'one2many':
                ...
                return tigernix.fields.Datetime.to_string(v)
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~tigernix.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~tigernix.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, descr in fields.items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = tigernix.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = tigernix.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return tigernix.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return tigernix.fields.Date.to_string(value)
...
def tagged(*tags):
    ...
    By default, all Test classes from tigernix.tests.common have a test_tags
