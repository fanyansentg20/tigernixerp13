...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  ...
                  version=tigernix.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('tigernix.tools.convert.init').error(
        ...
        # invoke method
        return tigernix.api.call_kw(model, method_name, args, kwargs)
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def __init__(self, cr, module, idref, mode, report=None, noupdate=False, xml_filename=None):
        self.mode = mode
        self.module = module
        self.envs = [tigernix.api.Environment(cr, SUPERUSER_ID, {})]
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <tigernixerp>, <tigernix> or <data>."
        try:
            self._tag_root(de)
    ...
    DATA_ROOTS = ['tigernix', 'data', 'tigernixerp']
